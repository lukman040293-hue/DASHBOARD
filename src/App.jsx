...
const MasterMapView = ({ allProjects, onSelectProject, mapType, isUIHidden, globalFeeds, supabaseClient, filterTahun }) => {
  ...
  const [expandedGlobalLayerId, setExpandedGlobalLayerId] = useState(null); // STATE BARU: Accordion Layer
  const [confirmDeleteGlobalId, setConfirmDeleteGlobalId] = useState(null); // STATE BARU: Konfirmasi Hapus
  ...
  // --- MENGHITUNG TOTAL ITEM UNTUK LEGENDA (Otomatis Terfilter) ---
  ...
  const globalLayerGroups = useMemo(() => {
     const groups = {};
     globalLayers.filter(l => l.isVisible !== false).forEach(layer => {
        ...
        if (showGlobalEditor && (globalInputMode === 'line' || globalInputMode === 'polygon')) {
          setGlobalLayers(prev => {
            ...
            if (newLayers.length === 0 || lastType !== globalInputMode) {
              ...
              setExpandedGlobalLayerId(newId); // Auto-expand layer baru
            }
      ...
      <div className={`absolute top-[85px] md:top-[90px] left-4 md:left-6 z-[20] flex flex-col items-start pointer-events-none transition-all duration-500 ease-in-out ${isUIHidden ? 'opacity-0 -translate-x-12' : 'opacity-100 translate-x-0'}`}>
        {showLegend ? (
          <div className="bg-white/80 backdrop-blur-md border border-slate-200 rounded-2xl shadow-xl p-5 pointer-events-auto w-max animate-in slide-in-from-left-4">
             <div className="flex justify-between items-start border-b border-slate-200 pb-3 mb-4 gap-4">
               <div>
                 ...
                 <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1.5 ml-5.5">
                   {filterTahun === 'Semua' ? 'Semua Tahun' : `Tahun ${filterTahun}`}
                 </p>
               </div>
               <button onClick={() => setShowLegend(false)} className="text-slate-500 hover:text-rose-500 transition-colors p-1.5 bg-slate-100/50 rounded-lg hover:bg-slate-200" title="Sembunyikan Legenda"><X size={14} /></button>
             </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
             ...
             <div className="space-y-3">
               {globalLayers.length === 0 ? (
                  ...
                  const isExpanded = expandedGlobalLayerId === layer.id;
                  
                  return (
                  <div key={layer.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden transition-all">
                     
                     {/* Header Layer (Buka / Tutup Accordion) */}
                     <div 
                         className="flex justify-between items-center p-3 cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors"
                         onClick={() => setExpandedGlobalLayerId(isExpanded ? null : layer.id)}
                     >
                        <div className="flex items-center gap-2">
                           <div className="w-3 h-3 rounded-[3px] shrink-0 border border-black/10 shadow-sm" style={{backgroundColor: layer.color || (layer.type === 'polygon' ? '#14b8a6' : '#0ea5e9')}}></div>
                           <span className={`font-bold text-xs truncate max-w-[120px] ${layer.isVisible === false ? 'text-slate-400 line-through opacity-70' : 'text-slate-700'}`} title={layer.name}>{layer.name || 'Area / Jalur'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                           <button
                              type="button"
                              onClick={(e) => {
                                 e.stopPropagation();
                                 setGlobalLayers(prev => prev.map(l => l.id === layer.id ? { ...l, isVisible: l.isVisible === false ? true : false } : l));
                              }}
                              className={`p-1.5 rounded-lg transition-colors shadow-sm border ${layer.isVisible !== false ? 'text-blue-500 hover:bg-blue-100 bg-white border-slate-200' : 'text-slate-400 hover:bg-slate-200 bg-slate-100 border-transparent'}`}
                              title={layer.isVisible !== false ? 'Sembunyikan Layer' : 'Tampilkan Layer'}
                           >
                              {layer.isVisible !== false ? <Eye size={14} /> : <EyeOff size={14} />}
                           </button>
                           <span className="text-[9px] font-bold text-slate-400 bg-white px-1.5 py-0.5 rounded shadow-sm border border-slate-200 uppercase">{layer.type === 'polygon' ? 'Poligon' : 'Garis'}</span>
                           {isExpanded ? <ChevronDown size={14} className="text-slate-500" /> : <ChevronRight size={14} className="text-slate-500" />}
                        </div>
                     </div>

                     {/* Body Form yang Tersembunyi */}
                     {isExpanded && (
                       <div className="p-3 border-t border-slate-100 bg-white animate-in slide-in-from-top-2 fade-in duration-200">
                         
                         {/* Input Nama Layer */}
                         <div className="mb-2.5">
                            <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Nama Layer</label>
                            <input 
                               type="text" 
                               value={layer.name} 
                               onChange={(e) => setGlobalLayers(prev => prev.map(l => l.id === layer.id ? { ...l, name: e.target.value } : l))}
                               className="font-bold text-slate-800 text-xs outline-none bg-slate-50 w-full p-2.5 rounded-lg border border-slate-200 focus:border-blue-400 focus:bg-white transition-all shadow-inner"
                               placeholder="Nama Area / Jalur"
                            />
                         </div>

                         {/* Pengaturan Gaya (Warna, Tipe, Style) */}
                         <div className="mb-2.5">
                            <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Tampilan Visual</label>
                         ...
                         {/* List Titik Vertex */}
                         <div className="mb-4">
                            <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest flex justify-between items-center mb-1">
                               <span>Titik Koordinat</span>
                               <span className="bg-slate-200 text-slate-600 px-1.5 py-[1px] rounded">{layer.points?.length || 0} Titik</span>
                            </label>
                            <div className="max-h-32 overflow-y-auto custom-scrollbar pr-1 bg-slate-50 border border-slate-100 rounded-lg p-1.5">
                              {(!layer.points || layer.points.length === 0) ? <div className="text-[9px] text-slate-400 italic text-center py-2">Belum ada titik digambar</div> : layer.points.map((p, i) => (
                                <div key={i} className="flex justify-between items-center mb-1 pl-2 border-l-2 text-slate-600 group/pt hover:bg-white rounded py-0.5" style={{ borderColor: layer.color || (layer.type === 'polygon' ? '#14b8a6' : '#0ea5e9') }}>
                         ...
                         {/* Tombol Hapus Layer & Konfirmasi Ganda */}
                         <div className="pt-2 border-t border-slate-100">
                             {confirmDeleteGlobalId === layer.id ? (
                                <div className="flex flex-col gap-2 p-2 bg-rose-50 border border-rose-100 rounded-lg animate-in zoom-in-95">
                                   <span className="text-[9px] font-bold text-rose-600 text-center uppercase tracking-widest">Yakin hapus layer ini?</span>
                                   <div className="flex gap-2">
                                       <button onClick={() => {
                                          setGlobalLayers(prev => prev.filter(l => l.id !== layer.id));
                                          setConfirmDeleteGlobalId(null);
                                          setExpandedGlobalLayerId(null);
                                       }} className="flex-1 py-2 bg-rose-500 text-white rounded text-[10px] font-bold shadow-sm hover:bg-rose-600 transition-colors">Ya, Hapus</button>
                                       <button onClick={() => setConfirmDeleteGlobalId(null)} className="flex-1 py-2 bg-white border border-slate-200 text-slate-600 rounded text-[10px] font-bold hover:bg-slate-100 transition-colors">Batal</button>
                                   </div>
                                </div>
                             ) : (
                                <button onClick={() => setConfirmDeleteGlobalId(layer.id)} className="w-full py-2 bg-rose-50 text-rose-500 hover:bg-rose-100 hover:text-rose-600 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1.5 transition-colors border border-transparent hover:border-rose-200">
                                   <Trash size={12} /> Hapus Layer dari Peta
                                </button>
                             )}
                         </div>
             ...
             <div className="flex gap-2 mt-4">
               <button onClick={() => {
                   const newId = `global-${Date.now()}`;
                   setGlobalLayers(prev => [...prev, { id: newId, name: `Jalur Air ${prev.length + 1}`, type: 'line', color: '#0ea5e9', isDashed: false, points: [], isVisible: true }]);
                   setExpandedGlobalLayerId(newId);
                   setGlobalInputMode('line');
               }} className="flex-1 text-cyan-600 bg-cyan-50 hover:bg-cyan-100 py-2.5 text-[10px] font-bold rounded-xl border border-cyan-200 transition-colors shadow-sm">+ Tambah Garis</button>
               <button onClick={() => {
                   const newId = `gpoly-${Date.now()}`;
                   setGlobalLayers(prev => [...prev, { id: newId, name: `Kolam Retensi ${prev.length + 1}`, type: 'polygon', color: '#14b8a6', isDashed: false, points: [], isVisible: true }]);
                   setExpandedGlobalLayerId(newId);
                   setGlobalInputMode('polygon');
               }} className="flex-1 text-teal-600 bg-teal-50 hover:bg-teal-100 py-2.5 text-[10px] font-bold rounded-xl border border-teal-200 transition-colors shadow-sm">+ Area</button>
             </div>
          </div>
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden font-sans bg-slate-900">
      ...
      <div className="absolute inset-0 z-0">
        <MasterMapView allProjects={filteredProjectsForMap} onSelectProject={onSelectProject} mapType={mapType} isUIHidden={isUIHidden} globalFeeds={globalFeeds} supabaseClient={supabaseClient} filterTahun={filterTahun} />
      </div>
  ...
  if (!projectData) {
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 flex-col gap-4">
        ...
        <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Memuat Dashboard Publik...</span>
      </div>
    );
  }

  return (
    <div className="flex flex-col h-screen bg-[#f4f7fe] overflow-y-auto custom-scrollbar font-sans text-slate-800">
      {/* Header Publik */}
      <div className="bg-blue-600 text-white p-6 md:px-10 md:py-8 shadow-md flex justify-between items-center shrink-0">
        <div>
          <h1 className="text-2xl md:text-3xl font-black tracking-tight">{projectData.pekerjaan}</h1>
          <p className="text-blue-200 text-xs uppercase tracking-widest font-bold mt-2">DASHBOARD INFORMASI PUBLIK (READ-ONLY)</p>
        </div>
        <Activity size={48} className="opacity-20 hidden md:block" />
      </div>

      {/* Konten Publik */}
      <div className="p-4 md:p-8 flex flex-col gap-6 max-w-7xl mx-auto w-full">
        
        {/* ROW 1: 3 Kartu Info Utama */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
           <CircularStatCard 
             label="Progress Fisik" icon={Activity} percentage={actualProg !== null ? actualProg : (projectData?.actual_progress || 0)} trend={true} isPositive={isDeviasiPositive} dropShadowColor="emerald" 
             subContent={
               <div className="flex flex-col items-center mt-1">
                 <div className="text-[10px] font-medium flex items-center gap-1.5 mt-1 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-100">
                   <span className="text-slate-400 uppercase tracking-wider">Deviasi:</span>
                   <span className={isDeviasiPositive ? "text-emerald-500 font-bold" : "text-rose-500 font-bold"}>{isDeviasiPositive ? '+' : ''}{deviasi}%</span>
                 </div>
               </div>
             } 
           />
           <CircularStatCard 
             label="TAGIHAN" icon={Banknote} percentage={terminPct ? parseFloat(terminPct) : 0} dropShadowColor="blue" 
             subContent={
               <div className="flex flex-col items-center mt-1">
                 <span className="text-[12px] md:text-sm font-bold uppercase tracking-wider text-slate-800">Termin {toRoman(String(terminNum))}</span>
               </div>
             } 
           />
        ...
        {/* ROW 2: Kurva S & Item Pekerjaan */}
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:h-[400px]">
          
          {/* Kurva S */}
          <div className="lg:col-span-7 bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex flex-col h-[350px] lg:h-full">
            <h3 className="text-sm font-black text-slate-800 mb-4 tracking-tight uppercase flex items-center gap-2"><TrendingUp className="text-blue-500" size={18}/> Kurva S Pekerjaan</h3>
            <div className="flex-1 w-full -ml-4 min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={processedSCurveData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                  <defs>
                    <linearGradient id="colorRencanaDashPub" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#06b6d4" stopOpacity={0.2}/><stop offset="95%" stopColor="#06b6d4" stopOpacity={0}/></linearGradient>
                    <linearGradient id="colorRealisasiDashPub" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.3}/><stop offset="95%" stopColor="#3b82f6" stopOpacity={0}/></linearGradient>
                  </defs>
                  ...
                  <Area type="monotone" dataKey="r" stroke="#06b6d4" strokeWidth={2} fill="url(#colorRencanaDashPub)" name="Rencana (M=Mingguan)" dot={{ r: 3, strokeWidth: 2, fill: '#ffffff' }} />
                  <Area type="monotone" dataKey="a" stroke="#3b82f6" strokeWidth={3} fill="url(#colorRealisasiDashPub)" name="Progres Fisik (M=Mingguan)" dot={{ r: 3.5, strokeWidth: 2, fill: '#ffffff' }} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          {/* Item Pekerjaan */}
          <div className="lg:col-span-5 bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex flex-col h-[350px] lg:h-full overflow-hidden">
            <h3 className="text-sm font-black text-slate-800 mb-4 tracking-tight uppercase flex items-center gap-2"><Ruler className="text-amber-500" size={18}/> Progress Item Pekerjaan</h3>
            <div className="flex-1 overflow-y-auto custom-scrollbar w-full">
              {(!projectData?.item_utama_data || projectData.item_utama_data.length === 0) ? (
                <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  <p className="text-xs font-bold text-slate-400">Belum ada rincian item utama.</p>
                </div>
              ) : (
                <table className="w-full text-left border-collapse">
                  <thead className="sticky top-0 bg-white z-20">
                    <tr className="border-b border-slate-200">
                      <th className="pb-3 text-xs font-black text-slate-500 uppercase">Item</th>
                      <th className="pb-3 text-xs font-black text-slate-500 uppercase text-right">Progress</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {projectData.item_utama_data.map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        <td className="py-4 pr-2">
                          <span className="text-xs font-bold text-slate-800 uppercase block" title={item.nama}>{item.nama}</span>
                          <span className="text-[10px] text-slate-500">Vol: {item.nilai || 0} {item.satuan || ''}</span>
                        </td>
                        <td className="py-4 pl-2 text-right">
                          <span className="text-base font-black text-blue-600">{item.persen}%</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {/* ROW 3: PETA LOKASI */}
        <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex flex-col h-[500px]">
          <h3 className="text-sm font-black text-slate-800 mb-4 tracking-tight uppercase flex items-center gap-2"><MapPin className="text-emerald-500" size={18}/> Peta & Rute Lokasi</h3>
          <div className="flex-1 relative rounded-2xl overflow-hidden border border-slate-200">
             {/* Menggunakan SiteMapView dengan mode isPublic agar tombol edit tersembunyi */}
             <SiteMapView projectData={projectData} isPublic={true} feeds={[]} />
          </div>
        </div>
...
const SiteMapView = ({ projectData, onUpdateRoutes, isUpdating, showMsg, feeds, userRole, isPublic }) => {
  const [isMapLoaded, setIsMapLoaded] = useState(false);
  ...
  return (
    <div className="h-full flex flex-col relative overflow-hidden bg-slate-900 rounded-3xl">
      ...
      {showPlanEditor && userRole !== 'guest' && !isPublic && (
...
const ModeSelectionView = ({ projects, onSelectMaster, onSelectProject, onAddProject, onLogout, onViewAbsensi, onViewRekap, isModalOpen, globalFeeds, globalReadFeeds, onMarkAllGlobalRead, onGlobalNotifClick }) => {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#151515] font-sans relative overflow-hidden">
      ...
      <div className={`absolute left-1/2 -translate-x-1/2 z-[99999] flex flex-col items-center pointer-events-none w-full px-4 transition-all duration-500 ${isModalOpen ? 'top-2' : 'top-6 md:top-10'}`}>
        {/* Tombol Panah Tengah */}
        <button 
          onClick={() => setIsUIHidden(!isUIHidden)}
          className={`pointer-events-auto flex items-center justify-center text-white/80 hover:text-white hover:scale-125 transition-all z-20 drop-shadow-lg ${isModalOpen ? 'hidden' : ''}`}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Cek sesi saat pertama kali dimuat (Normal Flow)
    supabaseClient.auth.getSession().then(({ data: { session } }) => {
      if (session) {
        setIsLoggedIn(true);
  ...
  useEffect(() => { 
     // Jika mode publik, hentikan fetch global karena hanya perlu fetch detail 1 proyek
     if (supabaseClient && isLoggedIn && !isPublicRoute) {
        fetchAllProjects(); 
        fetchAttendances();
        fetchEmployees();
        fetchGlobalFeeds();
     }
  }, [supabaseClient, isLoggedIn, isPublicRoute]);
