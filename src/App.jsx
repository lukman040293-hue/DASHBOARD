...
const SUPABASE_ANON_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhueG9tb3Zjd3dqYnRpcnVwbmFvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc5MTQzMDEsImV4cCI6MjA5MzQ5MDMwMX0.TTqrps9cuHfJKNS-fHNjmrX1nza7Ktp-wDfbIi8Jhlk';
...
const INITIAL_AKTIVITAS = [
  ...
  {
    nama: 'Pek. Timbunan/pemadatan tanah',
    kemarin: '',
    hariIni: '',
    satuan: 'm',
  },
  ...
  {
    nama: 'Pek. Pengecoran plat saluran',
    kemarin: '',
    hariIni: '',
    satuan: 'm',
  },
...
const INITIAL_DINAS_DATA = [
  ...
  {
    type: 'personil',
    role: 'Pejabat Pelaksana Teknis Kegiatan (PPTK)',
    name: '',
    nip: '',
  },
  {
    type: 'personil',
    role: 'Kuasa Pengguna Anggaran (KPA)',
    name: '',
    nip: '',
  },
...
const findDynamicValue = (data, labels, keys = []) => {
  ...
  const field = data.fields.find((f) => {
    ...
    return (
      labels.some((kw) => l.includes(kw)) ||
      keys.some((kw) => String(f.id).toLowerCase().includes(kw))
    );
  });
...
const getFileIconInfo = (filename) => {
  const ext = String(filename || '')
    ...
    .toLowerCase();
  if (ext === 'pdf')
    return { bg: 'bg-rose-100', color: 'text-rose-600', icon: FileText };
...
// --- GENERATOR PDF ---
const generateDailyReportReceipt = async (
  reportData,
  projectData,
  reporterName = 'Admin Command Center'
) => {
  ...
  if (reportData.shifts && reportData.shifts.length > 0) {
    waktuStrPDF = reportData.shifts
      .map((s, i) => {
        const shiftTitle =
          reportData.shifts.length > 1 ? `Shift ${i + 1}: ` : '';
  ...
  const aktivitasTerisi = (reportData.aktivitas || []).filter(
    (act) =>
      (act.kemarin &&
        act.kemarin.toString().trim() !== '0' &&
        act.kemarin.toString().trim() !== '') ||
      (act.hariIni &&
        act.hariIni.toString().trim() !== '0' &&
        act.hariIni.toString().trim() !== '') ||
      (act.nama && act.nama.trim() !== '')
  );
  const aktivitasRows = aktivitasTerisi
    .map((act, i) => {
  ...
  for (let i = 0; i < cuacaKeys.length; i += 3) {
    const jam1 = cuacaKeys[i] || '';
    const stat1 = jam1 ? reportData.cuaca[jam1] : '';
    const jam2 = cuacaKeys[i + 1] || '';
    const stat2 = jam2 ? reportData.cuaca[jam2] : '';
    const jam3 = cuacaKeys[i + 2] || '';
    const stat3 = jam3 ? reportData.cuaca[jam3] : '';
  ...
  if (Array.isArray(reportData.tenagaKerja)) {
    tkValid = reportData.tenagaKerja.filter(
      (tk) => tk.posisi && tk.posisi.trim() !== ''
    );
  } else if (typeof reportData.tenagaKerja === 'object') {
    // Menangani format data lama
    tkValid = Object.keys(reportData.tenagaKerja || {}).map((k) => ({
      posisi: k,
      jumlah: reportData.tenagaKerja[k],
    }));
  }
  ...
  for (let i = 0; i < tkLeft.length; i++) {
    ...
    const countL = tkLeft[i].jumlah || 0;
    const posR = tkRight[i] ? tkRight[i].posisi : '';
    const countR = tkRight[i] ? tkRight[i].jumlah || 0 : '';
    tkRows2Col += `<tr>
      ...
      <td style="padding: 4px 6px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold;">${
        posR ? countR + ' Orang' : ''
      }</td>
    </tr>`;
  }
  ...
  container.innerHTML = `
    <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; line-height: 1.5;">
      <div style="text-align: center; margin-bottom: 25px; border-bottom: 2px solid #1e293b; padding-bottom: 15px;">
        ...
        <p style="margin: 5px 0 0; font-size: 14px; font-weight: bold; color: #475569;">${
          projectData?.pekerjaan || reportData.lokasi
        }</p>
      </div>

      <table style="width: 100%; margin-bottom: 20px; font-size: 13px;">
        <tr>
          <td style="width: 15%; font-weight: bold;">Tanggal</td><td style="width: 35%;">: ${
            reportData.tanggal
          }</td>
        ...
        <tr>
          ...
          <td style="font-weight: bold; vertical-align: top;">Lokasi</td><td style="vertical-align: top;">: ${
            reportData.lokasi || '-'
          }</td>
        </tr>
      </table>
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 15px;">
        ...
        ${
          aktivitasRows ||
          `<tr><td colspan="6" style="padding: 10px; border: 1px solid #cbd5e1; text-align: center; color: #64748b;">Tidak ada aktivitas pekerjaan yang dilaporkan</td></tr>`
        }
      </table>
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 11px; margin-bottom: 15px;">
        ...
        ${
          tkRows2Col ||
          `<tr><td colspan="4" style="padding: 10px; border: 1px solid #cbd5e1; text-align: center; color: #64748b;">Tidak ada tenaga kerja yang diinput</td></tr>`
        }
      </table>
      ...
      <div style="border: 1px solid #cbd5e1; padding: 15px; font-size: 12px; min-height: 60px; background-color: #f8fafc;">
        ${
          reportData.catatan
            ? reportData.catatan.replace(/\n/g, '<br/>')
            : '<i>Tidak ada catatan, kendala, atau saran.</i>'
        }
      </div>
  ...
  try {
    ...
    pdf.save(
      `Laporan_Harian_${reporterName.replace(/\s+/g, '_')}_${
        reportData.tanggal
      }.pdf`
    );
  } catch (err) {
...
const SurveyDetailRow = ({ label, value, isEven }) => (
  <div
    className={`flex flex-col sm:flex-row py-3 px-6 border-b border-slate-100 last:border-0 ${
...
const SurveyInputRow = ({ label, children }) => (
  <div>
    <label className="text-[10px] font-bold block mb-1.5 uppercase text-slate-500">
      {label}
    </label>
    {children}
  </div>
...
const CircularStatCard = ({
  ...
  const shadowClass =
    dropShadowColor === 'emerald'
      ? 'drop-shadow-[0_2px_4px_rgba(16,185,129,0.9)]'
      : 'drop-shadow-[0_2px_4px_rgba(59,130,246,0.9)]';
  ...
  return (
    <div className="bg-white p-4 md:p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col items-center relative h-full hover:shadow-md hover:-translate-y-1.5 transition-all duration-500 group overflow-hidden">
      <div
        className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-[40px] opacity-10 transition-opacity duration-700 group-hover:opacity-30 -mr-8 -mt-8 pointer-events-none ${
          dropShadowColor === 'emerald' ? 'bg-emerald-500' : 'bg-blue-500'
        }`}
      ></div>
      <div className="flex justify-between items-start w-full mb-1 relative z-10 text-left">
        <span className="text-slate-400 text-[11px] md:text-xs uppercase font-bold tracking-wider leading-tight mt-1">
          {String(label || '')}
        </span>
        <div
          className={`p-1.5 rounded-xl transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-3 shadow-sm ${
            dropShadowColor === 'emerald'
      ...
      <div className="relative flex items-center justify-center flex-1 w-full my-1 z-10">
        <svg
          viewBox="0 0 100 100"
          className={`transform -rotate-90 w-28 h-28 md:w-36 md:h-36 ${shadowClass} transition-transform duration-700 group-hover:scale-105`}
        >
          <circle
            cx="50"
            cy="50"
          ...
          <div className="flex items-center gap-0.5 drop-shadow-sm">
            {trend !== undefined && (
              <span
                className={
                  isPositive
                    ? 'text-emerald-500 text-[10px] md:text-xs'
                    : 'text-rose-500 text-[10px] md:text-xs'
                }
              >
                {isPositive ? '▲' : '▼'}
              </span>
            ...
            <span className="text-3xl md:text-4xl font-bold text-slate-800 tracking-tight">
              ...
              <span className="text-lg md:text-xl text-slate-800 font-bold ml-0.5">
                %
              </span>
            </span>
          </div>
...
const StatCard = ({ icon: Icon, label, value, sub, status, centered }) => (
  <div
    className={`bg-white p-4 md:p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col relative h-full hover:shadow-md hover:-translate-y-1.5 transition-all duration-500 group overflow-hidden text-left ${
    ...
    <div className="mt-1 flex flex-col items-center justify-center flex-1 w-full relative z-10">
      <h3 className="font-bold text-slate-800 tracking-tight leading-tight transition-transform duration-500 group-hover:scale-105 text-center flex flex-col items-center w-full">
        {typeof value === 'string' && value.includes('\n') ? (
          <>
            <span className="text-4xl md:text-5xl leading-none text-slate-800 font-bold">
              {value.split('\n')[0]}
            </span>
            <span className="text-sm md:text-base mt-2 text-slate-800 tracking-normal w-full px-2 font-bold">
              {value.split('\n')[1]}
            </span>
          </>
        ) : (
          <span className="text-4xl md:text-5xl text-slate-800 font-bold">
            {safeRender(value)}
          </span>
        )}
      </h3>
    ...
    <div className="mt-auto w-full relative z-10 pt-3">
      <div className="text-[11px] md:text-xs font-semibold flex flex-col items-center justify-center gap-1.5 px-3 py-2.5 bg-slate-50 rounded-xl border border-slate-100 w-full text-center">
        <div className="shrink-0">
          {status === 'warning' && (
            <span className="block w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse shadow-[0_0_8px_rgba(244,63,94,0.6)]" />
          )}
          {status === 'good' && (
            <span className="block w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]" />
          )}
          {status === 'neutral' && (
...
const FeedItem = ({ item, onView, onDelete, isUnread }) => {
  ...
  return (
    <div
      onClick={() => onView(item)}
      className={`py-3.5 group relative flex items-start gap-3 transition-all px-2 -mx-2 hover:bg-slate-50/80 rounded-xl cursor-pointer ${
        isUnread ? 'bg-blue-50/40 border border-blue-100/50 shadow-sm mb-1' : ''
      }`}
    >
      {/* Ikon Tipe Aktivitas atau Thumbnail Avatar */}
      <div
        className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-0.5 border shadow-sm overflow-hidden ${
          isProblem
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-center mb-0.5 gap-2 pr-6">
          <h4
            className={`text-xs font-bold truncate flex items-center ${
              isProblem
                ? 'text-rose-600'
          ...
          >
            {item.title}
            {isUnread && (
              <span className="ml-2 inline-block px-1.5 py-[2px] bg-rose-500 text-white text-[8px] font-black uppercase tracking-wider rounded-md shadow-sm animate-pulse">
                Baru
              </span>
            )}
          </h4>
        ...
        <p className="text-[11px] text-slate-500 font-medium truncate pr-6">
          {cleanDesc}
        </p>
          ...
          <div className="mt-2.5 flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest flex items-center gap-1.5">
                ...
                {mediaCount > 1 ? `${mediaCount} Lampiran` : 'Ada Lampiran'}
              </span>
      ...
      {onDelete && (
        <button
          ...
          className="absolute top-3.5 right-2 p-1.5 bg-white shadow-sm border border-rose-100 text-rose-500 rounded-lg opacity-0 group-hover:opacity-100 transition-all hover:bg-rose-50 hover:scale-110 z-10"
          title="Hapus Log"
        >
          <Trash size={12} />
        </button>
...
const SChartTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    const hasActual =
      actualObj &&
      actualObj.value !== null &&
      actualObj.value !== undefined &&
      actualObj.value !== '';
    const actual = hasActual ? actualObj.value : null;
    const dev = hasActual ? (actual - plan).toFixed(2) : '0.00';
    const devColor =
      hasActual && actual >= plan ? 'text-emerald-500' : 'text-rose-500';
    const devSign = hasActual && actual > plan ? '+' : '';

    let displayLabel =
      typeof label === 'object' ? JSON.stringify(label) : String(label || '');
    if (displayLabel.startsWith('M') || displayLabel.startsWith('W'))
      displayLabel = `Minggu Ke-${displayLabel.substring(1)}`;
    ...
    return (
      <div className="bg-white border border-slate-100 p-4 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] text-left z-50 relative">
        <p className="text-xs font-bold text-slate-800 mb-3 tracking-widest border-b border-slate-100 pb-2 uppercase">
          {displayLabel}
        </p>
        <div className="space-y-2">
          ...
          {hasActual ? (
            <>
              ...
              <div className="pt-2 mt-2 border-t border-slate-100 flex justify-between gap-8 text-[11px]">
                <span className="text-slate-400 font-bold uppercase tracking-tighter">
                  Deviasi:
                </span>
                <span className={`font-black ${devColor}`}>
                  {devSign}
                  {dev}%
                </span>
              </div>
            ...
            <div className="pt-2 mt-2 border-t border-slate-100 flex justify-between gap-8 text-[11px]">
              <span className="text-slate-400 font-bold uppercase tracking-tighter italic">
                Progres belum diupdate
              </span>
            </div>
          )}
        </div>
...
const AbsensiView = ({
  ...
  const rekapData = useMemo(() => {
    ...
    (attendances || []).forEach((abs) => {
      ...
      if (
        absDate.getMonth() === targetMonth &&
        absDate.getFullYear() === targetYear
      ) {
        const empName = abs.employee_name;
        if (rekapMap.has(empName)) {
          const current = rekapMap.get(empName);
          current.hadir += 1;
          if (String(abs.status || '').includes('Terlambat'))
            current.terlambat += 1;
          if (String(abs.status || '').includes('Masuk - Tepat Waktu'))
            current.tepatWaktu += 1;
          if (String(abs.status || '').includes('Lebih Awal'))
            current.pulangAwal += 1;
        } else {
          ...
          rekapMap.set(empName, {
            ...
            tepatWaktu: String(abs.status || '').includes('Masuk - Tepat Waktu')
              ? 1
              : 0,
  ...
  const handleExportCSV = () => {
    ...
    csvContent +=
      'ID Karyawan,Nama Lengkap,Jabatan,Total Hadir (Hari),Masuk Tepat Waktu,Terlambat,Pulang Lebih Awal\n';
    ...
    <div className="flex flex-col h-screen w-full bg-slate-50 font-sans relative">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[2000] p-4">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-sm shadow-2xl relative text-center">
            ...
            <p className="text-xs text-slate-500 mb-8 font-medium leading-relaxed">
              Data absensi atas nama <b>{deleteConfigAbsensi.name}</b> ini akan
              dihapus secara permanen.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteConfigAbsensi(null)}
                className="flex-1 py-3.5 bg-slate-100 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-200 transition-colors"
              >
                Batal
              </button>
              <button
                ...
                className="flex-1 py-3.5 bg-rose-500 text-white rounded-xl text-sm font-bold hover:bg-rose-600 transition-colors shadow-md"
              >
                {isProcessing ? (
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">
              REKAPITULASI ABSENSI & DATA KARYAWAN
            </p>
          </div>
        </div>
        <button
          onClick={onRefresh}
          disabled={isProcessing}
          className="p-2.5 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-xl border border-blue-100 transition-colors flex items-center gap-2 shadow-sm"
      ...
      <div className="bg-white border-b border-slate-200 px-6 sm:px-10 flex gap-6 shrink-0 shadow-sm relative z-0 overflow-x-auto custom-scrollbar">
        <button
          ...
          className={`py-4 text-[11px] font-black uppercase tracking-widest border-b-2 transition-all flex items-center gap-2 whitespace-nowrap ${
            activeTab === 'log'
              ? 'border-emerald-500 text-emerald-600'
              : 'border-transparent text-slate-400 hover:text-slate-600'
          }`}
        >
          <Clock size={16} /> Log Kehadiran
        </button>
        <button
        ...
        >
          <FileSpreadsheet size={16} /> Rekap & Laporan
        </button>
        <button
              ...
              <div className="p-6 md:p-8 border-b border-slate-100 bg-[#f8fafc] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h3 className="text-lg font-black text-slate-800 flex items-center gap-2">
                    <Clock className="text-emerald-500" /> Log Kehadiran
                    Karyawan
                  </h3>
                  <p className="text-xs text-slate-500 font-bold mt-1">
                    Real-time dari Aplikasi Mobile Karyawan
                  </p>
                </div>
                        ...
                        >
                          <td className="p-4 pl-6">
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-black shadow-sm shrink-0 border ${
                                  abs.location_type === 'Kantor Pusat'
                                    ? 'bg-blue-50 text-blue-600 border-blue-100'
                                    : 'bg-amber-50 text-amber-600 border-amber-100'
                                }`}
                              >
                                {String(abs.employee_name || 'U')
                                  .charAt(0)
                                  .toUpperCase()}
                              </div>
                              <div className="flex flex-col">
                                <div className="font-black text-slate-800 text-sm group-hover:text-blue-600 transition-colors">
                                  {safeRender(abs.employee_name)}
                                </div>
                          ...
                          <td className="p-4">
                            <div className="flex flex-col items-center justify-center">
                              <span
                                className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest mb-1.5 shadow-sm border ${
                                  abs.location_type === 'Kantor Pusat'
                              ...
                              <span
                                className="text-[10px] font-bold text-slate-600 line-clamp-1 text-center max-w-[150px]"
                                title={abs.location_name}
                              >
                                {safeRender(abs.location_name)}
                              </span>
                            </div>
                          </td>
                          <td className="p-4 text-center">
                            {abs.check_in_time ? (
                              <div className="flex flex-col items-center">
                                ...
                                {String(abs.status || '').includes(
                                  'Terlambat'
                                ) && (
                                  <span className="mt-1.5 px-2 py-0.5 bg-rose-100 text-rose-600 border border-rose-200 text-[8px] font-black uppercase rounded shadow-sm">
                                    Terlambat
                                  </span>
                                )}
                                {String(abs.status || '').includes(
                                  ...
                                  <span className="mt-1.5 px-2 py-0.5 bg-emerald-100 text-emerald-600 border border-emerald-200 text-[8px] font-black uppercase rounded shadow-sm">
                                    Tepat Waktu
                                  </span>
                                )}
                              </div>
                            ) : (
                              <span className="bg-rose-50 text-rose-500 border border-rose-100 px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-widest shadow-sm">
                                Tidak Absen
                              </span>
                            )}
                          </td>
                            ...
                            {abs.check_out_time ? (
                              <div className="flex flex-col items-center">
                                ...
                                {String(abs.status || '').includes(
                                  ...
                                  <span className="mt-1.5 px-2 py-0.5 bg-amber-100 text-amber-600 border border-amber-200 text-[8px] font-black uppercase rounded shadow-sm">
                                    Lebih Awal
                                  </span>
                                )}
                                {String(abs.status || '').includes(
                                  ...
                                  <span className="mt-1.5 px-2 py-0.5 bg-blue-100 text-blue-600 border border-blue-200 text-[8px] font-black uppercase rounded shadow-sm">
                                    Tepat Waktu
                                  </span>
                                )}
                              </div>
                          ...
                          <td className="p-4 text-center">
                            <div className="flex items-center justify-center gap-2">
                              {abs.latitude && abs.longitude && (
                                <a
                                  href={`https://www.google.com/maps/search/?api=1&query=${abs.latitude},${abs.longitude}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex p-2 bg-slate-50 border border-slate-200 hover:bg-blue-50 text-slate-500 hover:text-blue-600 rounded-xl transition-colors shadow-sm"
                                  title="Lihat di Maps"
                                >
                                  <MapPin size={16} />
                                </a>
          ...
          {activeTab === 'rekap' && (
            <>
              <div className="p-6 md:p-8 border-b border-slate-100 bg-[#f8fafc] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h3 className="text-lg font-black text-slate-800 flex items-center gap-2">
                    <FileSpreadsheet className="text-amber-500" /> Laporan
                    Kehadiran Bulanan
                  </h3>
                  <p className="text-xs text-slate-500 font-bold mt-1">
                    Rekap otomatis absensi karyawan untuk keperluan Payroll /
                    Penggajian
                  </p>
                </div>
                ...
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr className="text-[10px] font-black uppercase text-slate-500 tracking-wider">
                      ...
                      <th className="p-4 text-center text-rose-500">
                        Terlambat Masuk
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {rekapData.length === 0 ? (
                      <tr>
                        <td
                          colSpan="6"
                          className="p-12 text-center text-slate-400 font-bold text-xs"
                        >
                          Belum ada rekap kehadiran di bulan ini.
                        </td>
                      </tr>
                    ) : (
                      rekapData.map((rek, idx) => (
                        ...
                        >
                          <td className="p-4 pl-6 font-bold text-slate-400 text-xs text-center">
                            {idx + 1}
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4 text-center">
                            <span className="font-black text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
                              {rek.hadir}{' '}
                              <span className="text-[10px] uppercase font-bold text-blue-400 ml-1">
                                Hari
                              </span>
                            </span>
                          </td>
              ...
              {rekapData.length > 0 && (
                <div className="p-4 bg-slate-50 border-t border-slate-200 text-[10px] font-bold text-slate-500 text-center uppercase tracking-widest">
                  Laporan ini di-generate otomatis secara Real-Time berdasarkan
                  Log Kehadiran Aplikasi Mobile.
                </div>
              )}
            </>
          )}
          ...
          {activeTab === 'karyawan' && (
            <>
              <div className="p-6 md:p-8 border-b border-slate-100 bg-[#f8fafc] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h3 className="text-lg font-black text-slate-800 flex items-center gap-2">
                    <Users className="text-blue-500" /> Daftar Karyawan
                  </h3>
                  <p className="text-xs text-slate-500 font-bold mt-1">
                    Kelola data personil & akses login aplikasi mobile
                  </p>
                </div>
                <button
                  onClick={() => {
                  ...
                  className="bg-blue-600 text-white px-4 py-3 rounded-2xl flex items-center gap-2 text-[10px] font-bold uppercase shadow-sm hover:bg-blue-700 transition-colors"
                >
                  <UserPlus size={16} />{' '}
                    ...
                    {employees.length === 0 ? (
                      <tr>
                        ...
                        >
                          ...
                          <br />
                          Klik tombol "Tambah Karyawan" untuk menambahkan
                          personil.
                        </td>
                      </tr>
                    ) : (
                      employees.map((emp) => (
                        <tr
                          ...
                          className="hover:bg-slate-50/50 transition-colors"
                        >
                          <td className="p-4 pl-6 font-black text-slate-700 text-xs">
                            {emp.employee_id}
                          </td>
                          ...
                          <td className="p-4">
                            <span className="px-3 py-1.5 bg-slate-100 text-slate-600 text-[9px] font-black uppercase tracking-widest rounded-lg border border-slate-200">
                              {emp.role}
                            </span>
                          </td>
                          <td className="p-4 text-center">
                            <span className="font-mono text-slate-500 font-bold bg-slate-50 border border-slate-100 px-2.5 py-1 rounded-md text-xs tracking-[0.3em]">
                              {emp.pin}
                            </span>
                          </td>
                          <td className="p-4 text-center">
                            <div className="flex justify-center gap-2">
                              <button
                                ...
                                className="p-1.5 text-blue-500 bg-white border border-slate-200 shadow-sm hover:bg-blue-50 rounded-xl transition-colors"
                              >
                                <Edit3 size={14} />
                              </button>
                              <button
                                ...
                                className="p-1.5 text-rose-500 bg-white border border-slate-200 shadow-sm hover:bg-rose-50 rounded-xl transition-colors"
                              >
                                <Trash size={14} />
                              </button>
                            </div>
...
const MasterMapView = ({ allProjects, onSelectProject, mapType }) => {
  ...
  useEffect(() => {
    if (isMapLoaded && mapContainerRef.current && !mapInstanceRef.current) {
      ...
      markerLayerRef.current = window.L.featureGroup().addTo(
  ...
  useEffect(() => {
    if (isMapReady && mapInstanceRef.current) {
      ...
      let url =
        'https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}&apistyle=s.t%3A2%7Cp.v%3Aoff'; // Satelit Default
      let attribution = '';
                     ...
                     <div class="bg-white/80 backdrop-blur-[70px] rounded-2xl shadow-2xl border border-white/60 overflow-hidden relative w-full p-4">
                        ...
                        <div class="flex justify-between items-end border-t border-slate-300/40 pt-3">
                           ...
                           <div class="flex flex-col text-right pl-6">
                              ...
                              <span class="text-2xl font-black leading-none drop-shadow-sm" style="color: ${hexColor}">${actualProg.toFixed(
            1
...
const MasterDashboardView = ({
  ...
  const [mapType, setMapType] = useState(
    () => localStorage.getItem('master_mapType') || 'satellite'
  );
  const [isUIHidden, setIsUIHidden] = useState(true);

  useEffect(() => {
    localStorage.setItem('master_mapType', mapType);
  }, [mapType]);

  // --- KALKULASI RINGKASAN PORTOFOLIO KESELURUHAN ---
  const totalProjects = allProjects.length;
  const runningProjects = allProjects.filter(
    (p) => p.status === 'Running' || parseFloat(p.actual_progress || 0) > 0
  ).length;
  const avgProgress =
    totalProjects > 0
      ? allProjects.reduce(
          (sum, p) => sum + parseFloat(p.actual_progress || 0),
          0
        ) / totalProjects
      : 0;
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden font-sans bg-slate-900">
      ...
      <button
        ...
        title={
          isUIHidden ? 'Tampilkan Menu Utama' : 'Sembunyikan Menu (Layar Penuh)'
        }
      >
        {isUIHidden ? (
          <ChevronDown
            size={20}
            className="md:w-6 md:h-6 group-hover:translate-y-1 transition-transform"
          />
        ) : (
          <ChevronUp
            size={20}
      ...
      {/* FLOATING HEADER */}
      <header
        className={`absolute top-4 left-4 right-4 md:top-8 md:left-6 md:right-6 z-20 flex justify-between items-start pointer-events-none transition-all duration-500 ease-in-out ${
          isUIHidden ? '-translate-y-32 opacity-0' : 'translate-y-0 opacity-100'
        }`}
      >
        ...
        <div className="flex flex-col md:flex-row items-end md:items-center gap-2 md:gap-3 pointer-events-auto">
          ...
          <button
            ...
            className="bg-white/40 backdrop-blur-lg text-slate-800 px-3 py-3 md:px-4 md:py-3.5 rounded-2xl text-[10px] md:text-xs font-black uppercase flex items-center gap-2 shadow-lg hover:bg-white/60 hover:text-blue-600 transition-colors border border-white/40"
          ...
          <button
            ...
            className="bg-blue-600 text-white px-3 py-3 md:px-4 md:py-3.5 rounded-2xl text-[10px] md:text-xs font-black uppercase flex items-center gap-2 shadow-lg hover:bg-blue-700 hover:scale-105 transition-all border border-blue-500"
          >
            <Plus size={16} />{' '}
            <span className="hidden sm:inline">Kamar Baru</span>
          </button>
        </div>
      </header>
      ...
      <div
        className={`absolute bottom-6 left-4 md:left-6 z-20 pointer-events-none transition-all duration-500 ease-in-out ${
          isUIHidden ? 'translate-y-48 opacity-0' : 'translate-y-0 opacity-100'
        }`}
      >
        <div className="bg-white/40 backdrop-blur-lg p-5 rounded-[24px] shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-white/40 pointer-events-auto flex flex-col gap-4 min-w-[240px]">
          ...
          <div className="flex flex-col pt-2 border-t border-slate-200/60">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-sm font-black text-emerald-600 drop-shadow-sm">
...
const PresentationView = ({
    ...
    return (
      <div className="p-8 flex flex-col items-center justify-center h-screen w-full text-center bg-slate-50">
        ...
        <p className="text-sm text-slate-500 font-medium mb-8">
          Pilih proyek terlebih dahulu untuk memulai presentasi.
        </p>
        <button
          onClick={onExit}
          className="bg-slate-100 text-slate-700 hover:bg-slate-200 px-8 py-3.5 rounded-2xl font-bold uppercase text-xs tracking-widest transition-colors"
        >
  ...
  return (
    <div
      ref={containerRef}
      className="flex flex-col h-screen w-full bg-slate-900 text-white relative overflow-hidden font-sans select-none"
    >
      {}
      <div
        className={`flex justify-between items-center p-6 shrink-0 transition-opacity duration-300 ${
          isFullscreen
            ? 'opacity-0 hover:opacity-100 absolute top-0 left-0 right-0 z-50 bg-slate-900/80 backdrop-blur-md'
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="text-xs font-bold text-slate-400 mr-4">
      ...
      <div className="flex-1 relative flex items-center justify-center p-8 md:p-16 overflow-hidden">
        {/* Slide 0: Cover */}
        <div
          className={`absolute inset-0 flex flex-col items-center justify-center p-12 transition-all duration-700 transform ${
            currentSlide === 0
        ...
        {/* Slide 1: Kurva S */}
        <div
          className={`absolute inset-0 flex flex-col items-center justify-center p-8 md:p-16 transition-all duration-700 transform w-full h-full ${
            currentSlide === 1
              ? 'opacity-100 translate-x-0'
              : currentSlide > 1
              ? 'opacity-0 -translate-x-full'
        ...
        >
          <h3 className="text-2xl md:text-3xl font-black text-white mb-8 shrink-0 flex items-center gap-3">
            <TrendingUp className="text-blue-500" /> KURVA S & PROGRES FISIK
          </h3>
        ...
        >
          <h3 className="text-2xl md:text-3xl font-black text-white mb-8 shrink-0 flex items-center gap-3">
            <Ruler className="text-blue-500" /> ITEM PEKERJAAN UTAMA
          </h3>
          <div className="w-full max-w-5xl bg-slate-800/50 rounded-[32px] border border-slate-700 p-8 shadow-xl overflow-y-auto custom-scrollbar">
            {!projectData?.item_utama_data ||
            projectData.item_utama_data.length === 0 ? (
              <div className="text-center py-12 text-slate-400">
                Belum ada item utama
              </div>
            ) : (
              <div className="space-y-4">
                {projectData.item_utama_data.map((item, idx) => (
                  <div
                    key={idx}
                    className="bg-slate-800 p-5 rounded-2xl border border-slate-700 flex flex-col gap-3"
                  >
                    <div className="flex justify-between items-center">
                      <h4 className="text-lg font-bold text-white">
                        {item.nama}
                      </h4>
                      <span className="text-xs font-black text-blue-400 bg-blue-900/30 px-3 py-1 rounded-lg">
                        Renc. Hari Ini: {item.bobot || '-'}
                      </span>
                    </div>
        ...
        >
          <h3 className="text-2xl md:text-3xl font-black text-white mb-8 shrink-0 flex items-center gap-3">
            <ImageIcon className="text-blue-500" /> DOKUMENTASI LAPANGAN
          </h3>
            ...
            {photos && photos.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {photos.slice(0, 6).map((photo, idx) => (
                  <div
                    key={idx}
                    className="aspect-video rounded-2xl overflow-hidden border border-slate-700 bg-slate-800 relative group shadow-lg"
                  >
              ...
              <div className="text-center py-20 text-slate-400 flex flex-col items-center">
                ...
                <span className="text-sm font-bold uppercase tracking-widest">
                  Belum ada dokumentasi foto lapangan
                </span>
              </div>
            )}
          </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 z-40 bg-slate-800/80 backdrop-blur-md p-2 rounded-2xl border border-slate-700 shadow-2xl">
        <button
          ...
          className="p-3 text-white disabled:opacity-30 disabled:cursor-not-allowed hover:bg-slate-700 rounded-xl transition-colors"
        >
          <ChevronLeft size={24} />
        </button>
...
const RABView = () => (
  <div className="p-8 flex flex-col items-center justify-center h-full text-center">
    ...
    <p className="text-sm text-slate-500 font-medium mb-8">
      Modul Rencana Anggaran Biaya sedang dalam tahap penyempurnaan.
    </p>
  </div>
);
...
const THEME_COLORS = {
  amber: {
    plan: 'bg-amber-200 border-amber-300',
    actualBg: 'bg-amber-100 border-amber-300',
    actualFill: 'bg-amber-500',
    legendMarker: 'bg-amber-400 border-amber-500',
  },
  blue: {
    plan: 'bg-blue-200 border-blue-300',
    actualBg: 'bg-blue-100 border-blue-300',
    actualFill: 'bg-blue-500',
    legendMarker: 'bg-blue-500 border-blue-600',
  },
  emerald: {
    plan: 'bg-emerald-200 border-emerald-300',
    actualBg: 'bg-emerald-100 border-emerald-300',
    actualFill: 'bg-emerald-500',
    legendMarker: 'bg-emerald-500 border-emerald-600',
  },
  rose: {
    plan: 'bg-rose-200 border-rose-300',
    actualBg: 'bg-rose-100 border-rose-300',
    actualFill: 'bg-rose-500',
    legendMarker: 'bg-rose-500 border-rose-600',
  },
  purple: {
    plan: 'bg-purple-200 border-purple-300',
    actualBg: 'bg-purple-100 border-purple-300',
    actualFill: 'bg-purple-500',
    legendMarker: 'bg-purple-500 border-purple-600',
  },
  slate: {
    plan: 'bg-slate-200 border-slate-300',
    actualBg: 'bg-slate-100 border-slate-300',
    actualFill: 'bg-slate-500',
    legendMarker: 'bg-slate-500 border-slate-600',
  },
};
...
const GanttChartView = ({ projectData, onSaveSchedule, isProcessing }) => {
  ...
  useEffect(() => {
    if (
      projectData &&
      Array.isArray(projectData.schedule_data) &&
      projectData.schedule_data.length > 0
    ) {
  ...
  const handleExportExcel = () => {
      ...
      <body>
        <table>
          <tr>
            <th colspan="${
              7 + timelineInfo.days
            }" class="head-title">JADWAL PELAKSANAAN & TIMELINE - ${
      projectData?.pekerjaan || 'PROYEK'
    }</th>
          </tr>
          <tr>
            ...
            <th colspan="${
              timelineInfo.days
            }" class="header">Timeline Pelaksanaan (Rentang: ${
      timelineInfo.days
    } Hari)</th>
          </tr>
          <tr>
    ...
    for (let i = 0; i < timelineInfo.days; i++) {
      ...
      html += `<th class="header" style="width: 25px; font-size: 9px; mso-number-format:'\@';">${d.getDate()}/${
        d.getMonth() + 1
      }</th>`;
    }
    html += '</tr>';
    ...
    tasks.forEach((t, i) => {
      ...
      html += `
        <tr>
          ...
          <td rowspan="2" style="text-align: center; vertical-align: middle; font-weight: bold; color: #0f172a;">${
            t.progress || 0
          }%</td>
          <td style="text-align: center; color: #64748b; font-size: 10px; background-color: #fffbeb;">${
            t.start || '-'
          }</td>
          <td style="text-align: center; color: #64748b; font-size: 10px; background-color: #fffbeb;">${
            t.end || '-'
          }</td>
          <td rowspan="2" style="text-align: center; vertical-align: middle; font-size: 10px; background-color: #eff6ff; font-weight: bold;">${
            t.actualStart || '-'
          }</td>
          <td rowspan="2" style="text-align: center; vertical-align: middle; font-size: 10px; background-color: #eff6ff; font-weight: bold;">${
            t.actualEnd || '-'
      ...
      for (let j = 0; j < timelineInfo.days; j++) {
        ...
        let bgClass = '';
        if (pStart && pEnd && tTime >= pStart && tTime <= pEnd)
          bgClass = 'plan-bg';
      ...
      for (let j = 0; j < timelineInfo.days; j++) {
        ...
        const tTime = d.getTime();
        let bgClass = '';
        if (aStart && aEnd && tTime >= aStart && tTime <= aEnd)
          bgClass = 'actual-bg';
    ...
    const link = document.createElement('a');
    link.href = url;
    const projectName = projectData?.pekerjaan
      ? projectData.pekerjaan.replace(/[^a-z0-9]/gi, '_')
      : 'Proyek';
  ...
  const timelineInfo = useMemo(() => {
    ...
    tasks.forEach((t) => {
      const sP = parseDateLocal(t.start);
      if (sP) validStartDates.push(sP);
      const eP = parseDateLocal(t.end);
      if (eP) validEndDates.push(eP);
      const sA = parseDateLocal(t.actualStart);
      if (sA) validStartDates.push(sA);
      const eA = parseDateLocal(t.actualEnd);
      if (eA) validEndDates.push(eA);
    });
      ...
      <div className="px-6 py-6 md:px-10 flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 bg-white/50 backdrop-blur-md shrink-0 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 uppercase mt-1 tracking-widest font-bold">
              Timeline Proyek & Progres Pekerjaan
            </p>
          </div>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-none bg-emerald-600 text-white px-4 md:px-6 py-3 rounded-2xl text-[10px] font-bold uppercase shadow-md flex justify-center gap-2 items-center hover:bg-emerald-700 transition-all"
          >
            <FileSpreadsheet size={16} />{' '}
            <span className="hidden sm:inline">Export Excel</span>
          </button>
          <button
            onClick={() => onSaveSchedule(tasks)}
            disabled={isProcessing}
            className="flex-1 sm:flex-none bg-blue-600 text-white px-4 md:px-6 py-3 rounded-2xl text-[10px] font-bold uppercase shadow-md flex justify-center gap-2 items-center hover:bg-blue-700 transition-all"
          >
            {isProcessing ? (
            ...
            <span className="hidden sm:inline">Simpan Jadwal</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col p-4 md:p-8 gap-4 md:gap-6">
        ...
        <div className="flex bg-slate-200/50 p-1.5 rounded-2xl w-full sm:w-max mx-auto shadow-sm shrink-0">
          <button
            ...
            className={`flex-1 sm:flex-none px-8 py-3 text-[10px] md:text-xs font-black uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-2 ${
              activeTab === 'input'
          ...
          <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm flex flex-col flex-1 min-h-0 animate-in fade-in zoom-in-95 duration-300">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-[24px] shrink-0">
              <h4 className="text-sm font-black text-slate-700 uppercase tracking-wide flex items-center gap-2">
                <LayoutDashboard size={16} className="text-blue-500" /> Daftar
                Pekerjaan
              </h4>
              <button
                onClick={() =>
                ...
                className="text-[10px] font-bold bg-white border border-slate-200 text-blue-600 px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:bg-slate-50 shadow-sm"
              >
                ...
                <tbody className="divide-y divide-slate-100">
                  {tasks.length === 0 ? (
                    <tr>
                      <td
                        colSpan="6"
                        className="p-16 text-center text-slate-400 text-sm font-bold bg-slate-50/50 border border-dashed border-slate-200 m-4 rounded-2xl"
                      >
                        Belum ada jadwal. Klik "Tambah Item" untuk memulai
                        mengisi data.
                      </td>
                    </tr>
                  ) : (
                    tasks.map((t, i) => (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <input
                            type="text"
                            className="w-full bg-transparent border border-transparent hover:border-slate-200 focus:bg-white focus:border-blue-400 p-2.5 rounded-xl text-sm font-bold text-slate-700 outline-none transition-all"
                            value={t.name}
                            onChange={(e) =>
                              handleUpdateTask(i, 'name', e.target.value)
                            }
                        ...
                        <td className="p-4">
                          <div className="flex gap-2 items-center bg-slate-50 p-1.5 rounded-xl border border-slate-100">
                            <input
                              type="date"
                              className="w-full bg-transparent focus:bg-white focus:border-blue-400 p-2 rounded-lg text-[10px] font-bold text-slate-700 outline-none transition-all"
                              value={t.start || ''}
                              onChange={(e) =>
                                handleUpdateTask(i, 'start', e.target.value)
                              }
                              title="Rencana Mulai"
                            />
                            ...
                            <input
                              ...
                              value={t.end || ''}
                              onChange={(e) =>
                                handleUpdateTask(i, 'end', e.target.value)
                              }
                        ...
                        <td className="p-4">
                          <div className="flex gap-2 items-center bg-slate-50 p-1.5 rounded-xl border border-slate-100">
                            ...
                            <input
                              ...
                              value={t.actualEnd || ''}
                              onChange={(e) =>
                                handleUpdateTask(i, 'actualEnd', e.target.value)
                              }
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              min="0"
                              max="100"
                              className="w-20 bg-slate-50 border border-slate-200 focus:bg-white focus:border-blue-400 p-2.5 rounded-xl text-xs font-bold text-slate-700 outline-none transition-all text-center"
                              value={t.progress}
                              onChange={(e) =>
                                handleUpdateTask(i, 'progress', e.target.value)
                              }
                            />{' '}
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={() => handleMoveTask(i, -1)}
                              disabled={i === 0}
                              className="p-2 text-slate-400 bg-slate-50 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all disabled:opacity-30 disabled:hover:bg-slate-50 disabled:hover:text-slate-400"
                              title="Geser ke Atas"
                            >
                              <ArrowUp size={14} />
                            </button>
                            ...
                            <button
                              onClick={() =>
                                setTasks(
                                  tasks.filter((item) => item.id !== t.id)
                                )
                              }
                              className="p-2 text-rose-400 bg-rose-50 rounded-xl hover:bg-rose-100 transition-all"
                              title="Hapus"
                            >
                              <Trash size={14} />
                            </button>
        ...
        {activeTab === 'view' && (
          <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden animate-in fade-in zoom-in-95 duration-300">
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 z-10 shrink-0">
              <h4 className="text-sm font-black text-slate-700 uppercase tracking-wide flex items-center gap-2">
                <TrendingUp size={16} className="text-blue-500" /> Visualisasi
                Timeline
              </h4>
              <div className="flex flex-wrap items-center gap-2 md:gap-4">
                ...
                <div className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm text-[9px] font-bold text-slate-500 uppercase tracking-widest">
                  <span
                    className={`w-3 h-3 rounded-sm border ${
                      THEME_COLORS[theme.plan]?.legendMarker ||
                      THEME_COLORS.amber.legendMarker
                    }`}
                  ></span>
                  Rencana:
                  <select
                    value={theme.plan}
                    onChange={(e) => handleThemeChange('plan', e.target.value)}
                    className="ml-1 outline-none bg-transparent cursor-pointer font-black text-slate-700"
                  >
                ...
                <div className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm text-[9px] font-bold text-slate-500 uppercase tracking-widest">
                  <span
                    className={`w-3 h-3 rounded-sm border ${
                      THEME_COLORS[theme.actual]?.legendMarker ||
                      THEME_COLORS.blue.legendMarker
                    }`}
                  ></span>
                  Realisasi:
                  <select
                    value={theme.actual}
                    onChange={(e) =>
                      handleThemeChange('actual', e.target.value)
                    }
                    className="ml-1 outline-none bg-transparent cursor-pointer font-black text-slate-700"
                  >
                    <option value="blue">Biru</option>
            ...
            <div className="flex-1 overflow-auto relative custom-scrollbar flex p-4 bg-[#f8fafc]">
              <div className="flex flex-col sticky left-0 z-30 bg-white border-y border-l border-slate-200 shadow-[2px_0_10px_rgba(0,0,0,0.02)] w-[160px] md:w-[200px] shrink-0 rounded-l-xl">
                ...
                {tasks.map((t, i) => (
                  <div
                    key={t.id}
                    className="h-14 px-4 flex items-center border-b border-slate-100 text-[11px] font-bold text-slate-700 truncate group relative shrink-0"
                  >
                    <span className="truncate w-full block" title={t.name}>
                      {i + 1}. {t.name || 'Pekerjaan Baru'}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div
                className="flex flex-col relative bg-white border-y border-r border-slate-200 rounded-r-xl"
                style={{
                  width: `${timelineInfo.days * 28}px`,
                  minWidth: `${timelineInfo.days * 28}px`,
                }}
              >
                  ...
                  <div className="h-8 flex border-b border-slate-200 bg-slate-100 w-full shrink-0 rounded-tr-xl overflow-hidden">
                    {(() => {
                      ...
                      return months.map((m, idx) => (
                        <div
                          key={idx}
                          className="border-r border-slate-200 flex items-center justify-center text-[10px] font-black uppercase text-slate-600 bg-slate-200/50"
                          style={{
                            width: `${m.days * 28}px`,
                            minWidth: `${m.days * 28}px`,
                          }}
                        >
                  ...
                  <div className="h-10 flex border-b border-slate-100 bg-slate-50 w-full shrink-0">
                    {Array.from({ length: timelineInfo.days }).map((_, i) => {
                      ...
                      return (
                        ...
                        <div
                          key={i}
                          className={`w-[28px] shrink-0 border-r border-slate-100 flex flex-col items-center justify-center text-[9px] font-mono ${
                            isWeekend
                  ...
                  {tasks.map((t, i) => {
                    const getBarData = (startStr, endStr) => {
                      ...
                      const startDiff = Math.round(
                        (startDate - timelineInfo.start) / (1000 * 60 * 60 * 24)
                      );
                      const dur =
                        Math.round(
                      ...
                      return {
                        left: Math.max(0, startDiff * 28),
                        width: Math.max(0, dur * 28),
                        valid: true,
                        startDate,
                        endDate,
                      };
                    ...
                    return (
                      <div
                        key={t.id}
                        className="h-14 border-b border-slate-100/50 flex flex-col relative group shrink-0"
                      >
                        {/* Bar Rencana (Plan) Dinamis Diperbarui */}
                        {plan.valid && (
                          <div
                            className={`absolute h-3.5 rounded-sm top-2 opacity-90 shadow-sm cursor-pointer hover:opacity-100 hover:shadow-md hover:-translate-y-[1px] transition-all z-10 ${
                              THEME_COLORS[theme.plan]?.plan ||
                              THEME_COLORS.amber.plan
                            }`}
                            style={{
                              left: plan.left + 'px',
                              width: plan.width + 'px',
                            }}
                            title={`Rencana: ${plan.startDate.toLocaleDateString(
                              'id-ID'
                            )} s/d ${plan.endDate.toLocaleDateString('id-ID')}`}
                          />
                        )}
                        ...
                        {actual.valid && (
                          <div
                            className={`absolute h-5 rounded-md bottom-2 overflow-hidden shadow-sm group-hover:shadow-md transition-all group-hover:-translate-y-[1px] cursor-pointer z-10 ${
                              THEME_COLORS[theme.actual]?.actualBg ||
                              THEME_COLORS.blue.actualBg
                            }`}
                            style={{
                              left: actual.left + 'px',
                              width: actual.width + 'px',
                            }}
                            title={`Realisasi: ${actual.startDate.toLocaleDateString(
                              'id-ID'
                            )} s/d ${actual.endDate.toLocaleDateString(
                              'id-ID'
                            )} (Progres: ${t.progress}%)`}
                          >
                            <div
                              className={`h-full transition-all duration-500 ${
                                THEME_COLORS[theme.actual]?.actualFill ||
                                THEME_COLORS.blue.actualFill
                              }`}
                              style={{
                                width:
  ...
  const photos = useMemo(() => {
    ...
    if (filterDate) {
      filtered = filtered.filter((f) => {
        ...
        const dStr = `${localD.getFullYear()}-${pad(
          localD.getMonth() + 1
        )}-${pad(localD.getDate())}`;
    ...
    <div className="h-full flex flex-col">
      <header className="px-6 py-6 md:px-10 flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 gap-4 bg-white/50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold">Galeri Dokumentasi</h2>
            <p className="text-[10px] text-slate-500 uppercase mt-0.5 tracking-widest font-bold">
              Kumpulan Foto & Video Lapangan
            </p>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <div className="flex items-center bg-white border border-slate-200 rounded-xl px-3 flex-1 sm:w-64 shadow-sm">
            ...
            <input
              ...
              className="w-full p-2 outline-none text-xs font-bold bg-transparent text-slate-700"
            />
          </div>
          <div className="flex items-center bg-white border border-slate-200 rounded-xl px-3 shadow-sm shrink-0">
            ...
            <input
              ...
              className="p-2 outline-none text-[11px] font-bold bg-transparent text-slate-700 uppercase"
            />
            {filterDate && (
              <button
                onClick={() => setFilterDate('')}
                className="text-rose-500 ml-2 hover:bg-rose-50 p-1 rounded transition-colors"
              >
                <X size={14} />
              </button>
            )}
      ...
      <div className="flex-1 p-6 md:p-10 overflow-y-auto custom-scrollbar">
        {photos.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
            {photos.map((item) => (
              <div
                key={item.display_id || item.id}
                className="group flex flex-col gap-3 bg-white p-3 rounded-[24px] border border-slate-100 shadow-sm hover:shadow-xl transition-all cursor-pointer relative"
                onClick={() => onView(item)}
              >
                <div className="h-32 sm:h-40 rounded-2xl overflow-hidden bg-slate-100 relative border border-slate-200/50">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 p-2 bg-white/90 text-rose-500 rounded-xl opacity-0 group-hover:opacity-100 transition-all z-10 hover:bg-rose-50 shadow-sm backdrop-blur-sm"
                  >
                    <Trash size={14} />
                  </button>
                  {item.is_problem && (
                    <div className="absolute bottom-2 left-2 bg-rose-500 text-white p-1 rounded-md shadow-md">
                      <AlertCircle size={12} />
                    </div>
                  )}
                </div>
                <div className="px-1">
                  ...
                  <h4 className="text-xs font-bold text-slate-800 line-clamp-2 leading-tight group-hover:text-blue-600 transition-colors">
                    {String(item.title || '')}
                  </h4>
                </div>
              </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-400">
            ...
            <p className="text-sm font-bold uppercase tracking-widest text-center">
              ...
              <br />
              <span className="text-[10px] font-medium opacity-70 mt-1 block">
                Silakan sesuaikan filter tanggal atau lakukan pencarian
              </span>
            </p>
          </div>
        )}
...
const SiteMapView = ({ projectData, onUpdateRoutes, isUpdating, showMsg }) => {
  const [isMapLoaded, setIsMapLoaded] = useState(false);
  ...
  useEffect(() => {
    localStorage.setItem('map_showDistances', JSON.stringify(showDistances));
  }, [showDistances]);
  useEffect(() => {
    localStorage.setItem('map_showPaths', JSON.stringify(showPaths));
  }, [showPaths]);
  useEffect(() => {
    localStorage.setItem(
      'map_showSketchLabels',
      JSON.stringify(showSketchLabels)
    );
  }, [showSketchLabels]);
  useEffect(() => {
    localStorage.setItem(
      'map_showSketchPoints',
      JSON.stringify(showSketchPoints)
    );
  }, [showSketchPoints]);
      ...
      try {
        ...
        for (let i = 0; i < placemarks.length; i++) {
          ...
          if (lineString || polygon) {
            ...
            const coordElement =
              targetElement.getElementsByTagName('coordinates')[0];
            if (coordElement) {
        ...
        if (importedPaths.length > 0) {
          ...
          if (showMsg)
            showMsg(
              `Berhasil mengimpor ${importedPaths.length} sketsa dari KML`,
              'success'
            );
          ...
          if (showMsg)
            showMsg(
              'Tidak ditemukan jalur atau poligon pada file KML ini',
              'warning'
            );
        }
      } catch (err) {
  ...
  const handleExportKML = () => {
      ...
      plannedPath.forEach((path) => {
        if (path.points && path.points.length > 0) {
          ...
          if (path.type === 'polygon') {
            ...
            kml += `        <Polygon><outerBoundaryIs><LinearRing><coordinates>${coords.join(
              ' '
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Peta_SynxBuild_${(
      projectData?.pekerjaan || 'Proyek'
    ).replace(/[^a-z0-9]/gi, '_')}.kml`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  useEffect(() => {
    if (projectData) {
      ...
      if (projectData.planned_path && Array.isArray(projectData.planned_path)) {
        if (
          projectData.planned_path.length > 0 &&
          !Array.isArray(projectData.planned_path[0]) &&
          !projectData.planned_path[0].points
        ) {
          ...
          initialPlannedPath = [
            {
              ...
              color: '#f59e0b',
              isDashed: true,
              points: projectData.planned_path,
            },
          ];
        } else if (
          projectData.planned_path.length > 0 &&
      ...
      if (
        projectData.actual_segments_data &&
        Array.isArray(projectData.actual_segments_data)
      )
        setActualSegments(projectData.actual_segments_data);
      else
        setActualSegments([
          {
            id: 1,
            name: 'Segmen 1',
            startLat: projectData.start_lat || '',
            startLng: projectData.start_lng || '',
            endLat: projectData.end_lat || '',
            endLng: projectData.end_lng || '',
          },
        ]);
    }
  }, [projectData]);
  ...
  useEffect(() => {
    if (isMapLoaded && mapContainerRef.current && !mapInstanceRef.current) {
      ...
      routeLayerRef.current = window.L.layerGroup().addTo(
        mapInstanceRef.current
      );
      surveyLayerRef.current = window.L.layerGroup().addTo(
        mapInstanceRef.current
      );
    }
  }, [isMapLoaded]);
  ...
  useEffect(() => {
    if (isMapLoaded && mapInstanceRef.current) {
      ...
      let url =
        'https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}&apistyle=s.t%3A2%7Cp.v%3Aoff'; // Default Google Satellite
      let attribution = '';
          ...
          setPlannedPath((prev) => {
            ...
            if (newPaths.length === 0 || lastType !== targetType) {
              newPaths.push({
                ...
                name:
                  targetType === 'line'
                    ? `Jalur ${newPaths.length + 1}`
                    : `Poligon ${newPaths.length + 1}`,
                type: targetType,
            ...
            newPaths[idx] = {
              ...
              points: [
                ...(newPaths[idx].points || []),
                {
                  lat: e.latlng.lat,
                  lng: e.latlng.lng,
                  sta: `T-${(newPaths[idx].points || []).length + 1}`,
                },
              ],
            };
            return newPaths;
  ...
  useEffect(() => {
      ...
      return window.L.divIcon({
        ...
        html: `<div class="relative flex flex-col items-center group cursor-pointer pointer-events-auto" style="transform: translate(-50%, -100%);">
                 <div class="absolute bottom-full mb-2.5 px-3 py-1.5 bg-white/95 backdrop-blur-md rounded-xl shadow-[0_8px_20px_rgba(0,0,0,0.15)] border border-slate-200/50 whitespace-nowrap text-center z-[9999] opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100 transition-all duration-300 pointer-events-none origin-bottom">
                    ...
                    <div class="text-[8.5px] font-bold text-slate-500 mt-1 tracking-wider font-mono">${Number(
                      lat
                    ).toFixed(6)}, ${Number(lng).toFixed(6)}</div>
                 </div>
    ...
    const createDistLabel = (text, isTotal, colorHex) =>
      window.L.divIcon({
        className: 'bg-transparent border-0 overflow-visible',
        html: `<div style="transform: translate(-50%, ${
          isTotal ? '-150%' : '-50%'
        }); background-color: ${
          isTotal ? colorHex : 'rgba(255,255,255,0.9)'
        }; color: ${
          isTotal ? '#fff' : colorHex
        }; border-color: ${colorHex};" class="w-max px-2.5 py-1 rounded-lg text-[10px] font-bold whitespace-nowrap border shadow-sm">${
          isTotal ? 'Total: ' : ''
        }${text}</div>`,
        iconSize: [0, 0],
      });
        ...
        if (coords.length > 0) {
          ...
          if (showSketchLabels) {
            ...
            window.L.marker(center, {
              ...
              icon: window.L.divIcon({
                className: 'bg-transparent border-0 overflow-visible',
                html: `<div style="transform: translate(-50%, -150%); background-color: rgba(255,255,255,0.95); color: ${
                  pathObj.color || (isPolygon ? '#10b981' : '#f59e0b')
                }; border: 2px ${pathObj.isDashed ? 'dashed' : 'solid'} ${
                  pathObj.color || (isPolygon ? '#10b981' : '#f59e0b')
                };" class="w-max px-3 py-1.5 rounded-xl text-[10px] font-black whitespace-nowrap shadow-lg uppercase tracking-wider backdrop-blur-md">${
          ...
          for (let i = 0; i < coords.length; i++) {
            ...
            if (showSketchPoints) {
              window.L.marker([coords[i][0], coords[i][1]], {
                ...
                icon: window.L.divIcon({
                  ...
                  html: `<div style="transform: translate(-50%, -50%); background-color: ${
                    pathObj.color || (isPolygon ? '#10b981' : '#f59e0b')
                  };" class="w-3 h-3 border-2 border-white rounded-full shadow-md"></div>`,
                  iconSize: [0, 0],
                }),
              }).addTo(routeLayerRef.current);
            }
            ...
            if (i < coords.length - 1) {
              ...
              if (showDistances)
                window.L.marker(
                  [(pt1.lat + pt2.lat) / 2, (pt1.lng + pt2.lng) / 2],
                  {
                    interactive: false,
                    zIndexOffset: 200,
                    icon: createDistLabel(
                      dist > 1000
    ...
    (actualSegments || []).forEach((seg, idx) => {
      ...
      if (!isNaN(sLat) && !isNaN(sLng) && !isNaN(eLat) && !isNaN(eLng)) {
        ...
        window.L.marker(coords[0], {
          icon: createMarker('Blue', `Awal`, seg.name, sLat, sLng),
          zIndexOffset: 5000,
        }).addTo(surveyLayerRef.current);
        window.L.marker(coords[1], {
          icon: createMarker('Red', `Akhir`, seg.name, eLat, eLng),
          zIndexOffset: 5000,
        }).addTo(surveyLayerRef.current);
    ...
    if (!showPlanEditor && inputMode === 'view' && hasActualData) {
      if (
        actualBounds &&
        typeof actualBounds.isValid === 'function' &&
        actualBounds.isValid()
      ) {
        if (!isInitialSiteFitDone.current) {
          mapInstanceRef.current.fitBounds(actualBounds, {
  ...
  return (
    <div
      className={`h-[70vh] md:h-full rounded-[32px] border border-slate-200 bg-slate-100 relative overflow-hidden flex flex-col shadow-sm ${
        inputMode === 'plan' ? 'cursor-crosshair' : ''
      }`}
    >
      ...
      <div className="hidden md:block absolute top-6 left-6 z-20 bg-white/95 backdrop-blur-md p-4 rounded-2xl border border-slate-100 shadow-md text-left pointer-events-auto max-w-[220px]">
        <h4 className="text-[10px] font-black uppercase text-slate-500 mb-3 tracking-wider">
          Legenda Peta
        </h4>
        <div className="space-y-3 text-[11px] font-bold text-slate-700 max-h-[40vh] overflow-y-auto custom-scrollbar pr-2">
          <div className="flex items-center gap-3">
            ...
            >
              <defs>
                <linearGradient id="lgBlue" x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <defs>
                <linearGradient id="lgRed" x1="0%" y1="0%" x2="0%" y2="100%">
            ...
            (plannedPath && plannedPath.length > 0 ? (
              plannedPath.map((path) => (
                <div key={path.id} className="flex items-center gap-3">
                  {path.type === 'polygon' ? (
                    <div
                      className="w-4 h-3 border-2 bg-opacity-30 shrink-0"
                      style={{
                        borderColor: path.color || '#10b981',
                        backgroundColor: path.color || '#10b981',
                      }}
                    ></div>
                  ) : (
                    <div
                      className="w-4 h-1 border-t-[3px] shrink-0"
                      style={{
                        borderColor: path.color || '#f59e0b',
                        borderStyle: path.isDashed ? 'dashed' : 'solid',
                      }}
                    ></div>
                  )}
                  <span
                  ...
                  >
                    {path.name ||
                      (path.type === 'polygon' ? 'Poligon' : 'Garis Sketsa')}
                  </span>
                </div>
              ))
            ) : (
      ...
      <div className="absolute top-4 right-4 md:top-6 md:right-6 z-20 flex flex-col items-end gap-2 pointer-events-auto">
        <button
          ...
          className={`bg-white p-2 sm:px-3 sm:py-2 sm:w-[130px] rounded-xl shadow-md text-[10px] sm:text-[11px] font-bold flex items-center justify-center sm:justify-start gap-2 border border-slate-100 hover:bg-slate-50 transition-all ${
            !showPaths ? 'text-slate-400' : 'text-blue-600'
          }`}
        >
          {showPaths ? (
        ...
        <button
          ...
          className="bg-white text-slate-700 p-2 sm:px-3 sm:py-2 sm:w-[130px] rounded-xl shadow-md text-[10px] sm:text-[11px] font-bold flex items-center justify-center sm:justify-start gap-2 border border-slate-100 hover:bg-slate-50 transition-all"
        >
          <Upload size={16} className="text-emerald-600 shrink-0" />{' '}
          <span className="hidden sm:inline truncate">Import KML</span>
        </button>

        <button
          onClick={handleExportKML}
          className="bg-emerald-600 text-white p-2 sm:px-3 sm:py-2 sm:w-[130px] rounded-xl shadow-md text-[10px] sm:text-[11px] font-bold flex items-center justify-center sm:justify-start gap-2 border border-emerald-500 hover:bg-emerald-700 transition-all"
        >
          <Download size={16} className="shrink-0" />{' '}
          <span className="hidden sm:inline truncate">Export KML</span>
        </button>
        ...
        {!showPlanEditor && (
          <button
            ...
            className="bg-blue-600 text-white p-2 sm:px-3 sm:py-2 sm:w-[130px] rounded-xl shadow-md text-[10px] sm:text-[11px] font-bold flex items-center justify-center sm:justify-start gap-2 border border-blue-500 hover:bg-blue-700 transition-all"
          >
            <Ruler size={16} className="shrink-0" />{' '}
            <span className="hidden sm:inline truncate">Editor Rute</span>
          </button>
        )}
      </div>
      ...
      {showPlanEditor && (
        <div className="absolute top-32 md:top-28 right-4 bottom-4 z-30 w-[300px] md:w-[360px] bg-white/95 backdrop-blur-xl border border-slate-200 rounded-3xl shadow-2xl flex flex-col pointer-events-auto">
          <button
            ...
            className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-rose-100 text-slate-500 hover:text-rose-600 rounded-full transition-colors z-40 shadow-sm"
            title="Tutup Editor"
          >
            <X size={16} />
          </button>
          <div className="p-4 md:p-5 border-b border-slate-100 shrink-0 relative">
            <h4 className="text-sm font-black text-slate-800 mb-3 md:mb-4 pr-8">
              Input Koordinat
            </h4>
            <div className="flex gap-2 bg-slate-100 p-1.5 rounded-xl border text-center">
              ...
              <button
                onClick={() => setInputMode('plan_polygon')}
                className={`flex-1 py-2.5 rounded-lg text-[10px] font-black transition-all ${
                  inputMode === 'plan_polygon'
          ...
          <div className="flex-1 overflow-y-auto p-4 text-left custom-scrollbar">
            {inputMode === 'plan' || inputMode === 'plan_polygon' ? (
              <div className="animate-in fade-in slide-in-from-left-2">
                <p
                  className={`text-[11px] mb-3 p-3 rounded-xl border ${
                    inputMode === 'plan_polygon'
                ...
                >
                  ...
                  {inputMode === 'plan_polygon' ? 'poligon' : 'garis'}.
                </p>
                <div className="flex justify-end gap-3 mb-2 flex-wrap">
                  <button
                    onClick={() => setShowSketchPoints(!showSketchPoints)}
                    className="text-[9px] font-bold text-emerald-500 hover:text-emerald-700"
                  >
                    {showSketchPoints ? 'Sembunyikan Titik' : 'Tampilkan Titik'}
                  </button>
                  <button
                    onClick={() => setShowSketchLabels(!showSketchLabels)}
                    className="text-[9px] font-bold text-amber-500 hover:text-amber-700"
                  >
                    {showSketchLabels ? 'Sembunyikan Label' : 'Tampilkan Label'}
                  </button>
                  <button
                    onClick={() => setShowDistances(!showDistances)}
                    className="text-[9px] font-bold text-blue-500 hover:text-blue-700"
                  >
                    {showDistances ? 'Sembunyikan Jarak' : 'Tampilkan Jarak'}
                  </button>
                </div>
                <div className="max-h-[300px] overflow-y-auto bg-slate-50 p-2 rounded-xl text-[10px] font-mono border border-slate-200 mb-3 custom-scrollbar">
                  {!plannedPath || plannedPath.length === 0 ? (
                    <div className="p-2 text-slate-500 text-center">
                      Belum ada titik rencana...
                    </div>
                  ) : (
                    plannedPath.map((pathObj) => (
                      <div
                        key={pathObj.id}
                        className="mb-3 bg-white p-2.5 rounded-lg border border-slate-200 shadow-sm relative group"
                      >
                        <div className="flex justify-between items-center mb-2">
                          ...
                          />
                          <button
                            onClick={() => handleDeletePath(pathObj.id)}
                            className="text-rose-500 bg-rose-50 p-1 rounded hover:bg-rose-100"
                          >
                            <Trash size={12} />
                          </button>
                        </div>
                        ...
                        <div className="pl-1">
                          {!pathObj.points || pathObj.points.length === 0 ? (
                            <div className="text-[9px] text-slate-400 italic">
                              Klik peta untuk menambah titik...
                            </div>
                          ) : (
                            pathObj.points.map((p, i) => (
                              <div
                                key={i}
                                className="flex justify-between items-center mb-1 pl-2 border-l-2 text-slate-600 group/pt hover:bg-slate-50 rounded"
                                style={{
                                  borderColor:
                                    pathObj.color ||
                                    (pathObj.type === 'polygon'
                              ...
                              >
                                <span>
                                  {p.sta}: {Number(p.lat).toFixed(4)},{' '}
                                  {Number(p.lng).toFixed(4)}
                                </span>
                                <button
                                  onClick={() =>
                                    handleDeletePoint(pathObj.id, i)
                                  }
                                  className="text-rose-400 opacity-0 group-hover/pt:opacity-100 p-0.5"
                                >
                                  <X size={10} />
                                </button>
                              </div>
                ...
                <div className="flex gap-2 mb-3">
                  <button
                    ...
                    className="flex-1 text-blue-600 bg-blue-50 py-2 text-[10px] font-bold rounded-xl border border-blue-100"
                  >
                    + Garis Baru
                  </button>
                  <button
                    ...
                    className="flex-1 text-emerald-600 bg-emerald-50 py-2 text-[10px] font-bold rounded-xl border border-emerald-100"
                  >
                    + Poligon
                  </button>
                </div>
                <button
                  ...
                  className="w-full text-rose-500 bg-rose-50 py-2 text-[10px] font-bold rounded-xl border border-rose-100"
                >
                  Reset Semua Sketsa
                </button>
              </div>
              ...
              <div className="animate-in fade-in slide-in-from-right-2 space-y-4">
                <div className="flex justify-between items-center">
                  <p className="text-[11px] text-cyan-700 bg-cyan-50 p-3 rounded-xl border border-cyan-100 w-full">
                    Input koordinat per segmen.
                  </p>
                </div>
                <div className="flex justify-end gap-3 mb-2 flex-wrap">
                  <button
                ...
                {(actualSegments || []).map((seg, idx) => (
                  <div
                    key={seg.id}
                    className="bg-white p-3 rounded-2xl border border-slate-200 relative group"
                  >
                    <div className="flex justify-between mb-3">
                      <input
                        type="text"
                        className="text-xs font-black bg-transparent w-3/4 outline-none"
                        value={seg.name}
                        onChange={(e) =>
                          handleUpdateSeg(seg.id, 'name', e.target.value)
                        }
                      />
                      {actualSegments && actualSegments.length > 1 && (
                        <button
                          onClick={() =>
                            setActualSegments((p) =>
                              (p || []).filter((s) => s.id !== seg.id)
                            )
                          }
                          className="text-rose-400 bg-rose-50 p-1 rounded"
                        >
                          <Trash size={12} />
                        </button>
                      )}
                    ...
                    <div className="space-y-2">
                      <div className="bg-slate-50 p-2 rounded-xl flex gap-2">
                        <input
                          type="number"
                          placeholder="Lat Awal"
                          value={seg.startLat}
                          onChange={(e) =>
                            setActualSegments((p) =>
                              (p || []).map((s) =>
                                s.id === seg.id
                                  ? { ...s, startLat: e.target.value }
                                  : s
                              )
                            )
                          }
                          className="w-full bg-white rounded p-1 text-xs"
                        />
                        <input
                          type="number"
                          placeholder="Lng Awal"
                          value={seg.startLng}
                          onChange={(e) =>
                            setActualSegments((p) =>
                              (p || []).map((s) =>
                                s.id === seg.id
                                  ? { ...s, startLng: e.target.value }
                                  : s
                              )
                            )
                          }
                      ...
                      <div className="bg-slate-50 p-2 rounded-xl flex gap-2">
                        <input
                          type="number"
                          placeholder="Lat Akhir"
                          value={seg.endLat}
                          onChange={(e) =>
                            setActualSegments((p) =>
                              (p || []).map((s) =>
                                s.id === seg.id
                                  ? { ...s, endLat: e.target.value }
                                  : s
                              )
                            )
                          }
                        ...
                        <input
                          type="number"
                          placeholder="Lng Akhir"
                          value={seg.endLng}
                          onChange={(e) =>
                            setActualSegments((p) =>
                              (p || []).map((s) =>
                                s.id === seg.id
                                  ? { ...s, endLng: e.target.value }
                                  : s
                              )
                            )
                          }
                ...
                <button
                  ...
                  className="w-full text-blue-600 bg-blue-50 py-2.5 text-[10px] font-bold flex justify-center gap-1 rounded-xl border border-blue-100"
                >
          ...
          <div className="p-4 md:p-5 border-t border-slate-100">
            <button
              onClick={() => onUpdateRoutes(plannedPath, actualSegments)}
              disabled={isUpdating}
              className="w-full bg-blue-600 text-white py-3.5 rounded-xl text-xs font-black uppercase shadow-md hover:bg-blue-700 transition-colors"
            >
              {isUpdating ? (
  ...
  return (
    <div className="mb-8">
      <div className="flex items-center gap-3 mb-4 px-1">
        <div
          className={`w-8 h-8 rounded-xl ${bgClass} ${colorClass} flex items-center justify-center shrink-0`}
        >
          {icon}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-300 overflow-hidden shadow-sm">
        <div className="hidden sm:flex bg-slate-200 border-b border-slate-300 py-3.5 px-6">
          <div className="w-1/3 text-[10px] font-black uppercase tracking-[0.15em] text-slate-800">
            Informasi / Entitas
          </div>
          <div className="w-2/3 text-[10px] font-black uppercase text-slate-800 pl-4 border-l border-slate-300">
        ...
        <div className="flex flex-col">
          {dataArray.map((item, idx) => (
            <div
              key={idx}
              className={`flex flex-col sm:flex-row py-4 px-6 border-b border-slate-200 last:border-0 ${
...
const LoginView = ({ onLogin, error, isProcessing }) => {
  ...
  return (
    <div className="flex h-screen w-full items-center justify-center font-sans relative overflow-hidden bg-white">
      ...
      <div className="w-full max-w-md bg-white p-8 md:p-10 rounded-[32px] shadow-2xl relative z-10 flex flex-col items-center border border-slate-100">
        ...
        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-8 text-center">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-5">
          <div>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 text-sm font-bold outline-none focus:border-blue-500 focus:bg-white transition-all"
                required
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={isProcessing}
            className="w-full bg-blue-600 text-white py-4 rounded-2xl text-sm font-black uppercase tracking-widest shadow-lg hover:bg-blue-500 transition-colors mt-4 flex items-center justify-center gap-2"
          >
            {isProcessing ? (
        ...
        <div className="mt-8 text-center border-t border-slate-100 pt-6 w-full">
          ...
          <p className="text-[10px] text-slate-500 mt-1.5 font-bold">
            User: <span className="text-slate-800 font-black">m</span> | Pass:{' '}
...
const ModeSelectionView = ({
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-slate-900 font-sans relative overflow-hidden">
      ...
      {/* Header */}
      <header
        className={`absolute top-0 left-0 right-0 flex justify-between items-center px-6 md:px-10 py-6 bg-transparent shrink-0 z-20 transition-all duration-500 ease-in-out ${
          isUIHidden
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl md:text-2xl font-normal tracking-tight text-white leading-none drop-shadow-sm">
              Dashboard
            </h1>
            <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mt-1 drop-shadow-sm">
              Gerbang Akses Utama
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 md:gap-3">
          <button
            ...
            className="p-3 bg-blue-500/20 text-blue-400 rounded-xl hover:bg-blue-500/30 hover:text-blue-300 transition-colors flex items-center justify-center shadow-sm backdrop-blur-md border border-blue-500/20"
            title="Buka Peta Induk"
          >
            <Globe2 size={20} />
          </button>
          <button
            onClick={onViewAbsensi}
            className="p-3 bg-emerald-500/20 text-emerald-400 rounded-xl hover:bg-emerald-500/30 hover:text-emerald-300 transition-colors flex items-center justify-center shadow-sm backdrop-blur-md border border-emerald-500/20"
            title="Data Absensi"
          >
            <Fingerprint size={20} />
          </button>
          <button
            onClick={onLogout}
            className="p-3 bg-rose-500/20 text-rose-400 rounded-xl hover:bg-rose-500/30 hover:text-rose-300 transition-colors flex items-center justify-center shadow-sm backdrop-blur-md border border-rose-500/20"
      ...
      {/* Konten Utama */}
      <div
        className={`flex-1 flex flex-col items-center justify-center p-6 md:p-10 relative z-10 transition-all duration-500 ${
          isUIHidden ? 'pt-6' : 'pt-32'
        }`}
      >
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-slate-800/95 backdrop-blur-xl rounded-[32px] p-8 border border-slate-700/50 shadow-[0_20px_40px_rgba(0,0,0,0.4)] hover:shadow-2xl hover:border-white/30 hover:-translate-y-1 transition-all duration-300 group flex flex-col items-center text-center relative overflow-hidden">
            ...
            <p className="text-base text-slate-200 font-medium leading-relaxed mb-8 relative z-10">
              Pilih Untuk Melihat Secara Detail
            </p>
...
const ProjectSelectionListView = ({
  projects,
  onSelectProject,
  onBack,
  onAddProject,
  ...
  const filteredProjects = projects.filter((p) => {
    ...
    const matchTitle = String(p.pekerjaan || '')
      .toLowerCase()
      .includes(query);
    ...
    <div className="flex flex-col h-screen w-full bg-slate-50 font-sans relative">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center px-6 md:px-10 py-6 bg-white shrink-0 z-20 shadow-sm border-b border-slate-200 gap-4">
        <div className="flex items-center gap-4 w-full md:w-auto">
          <button
            onClick={onBack}
            className="p-3 bg-slate-100 hover:bg-slate-200 rounded-xl text-slate-600 transition-colors shrink-0"
            title="Kembali ke Menu Utama"
          >
            <ArrowLeft size={20} />
          </button>
          <div className="flex flex-col">
            ...
            <p className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">
              Total {projects.length} Proyek Tersedia
            </p>
          </div>
        </div>
        <div className="flex w-full md:w-auto gap-3">
          <div className="flex-1 md:w-64 flex items-center bg-slate-100 border border-slate-200 rounded-xl px-3 shadow-inner">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full py-3 px-2 outline-none text-xs font-bold bg-transparent text-slate-700"
            />
          </div>
          <button
            onClick={onAddProject}
            className="bg-emerald-600 text-white px-4 py-3 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 shadow-md hover:bg-emerald-700 transition-all shrink-0"
          >
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar">
        ...
        <div className="max-w-7xl mx-auto mb-6 flex flex-wrap gap-2 md:gap-3">
          ...
          <button
            onClick={() => setStatusFilter('Pelaksanaan')}
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${
                statusFilter === 'Pelaksanaan' ? 'bg-blue-300' : 'bg-blue-500'
              }`}
            ></div>{' '}
            Pelaksanaan
          </button>
          <button
            onClick={() => setStatusFilter('Persiapan')}
            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm flex items-center gap-2 ${
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${
                statusFilter === 'Persiapan' ? 'bg-rose-200' : 'bg-rose-500'
              }`}
            ></div>{' '}
            Persiapan
          </button>
          ...
          {filteredProjects.length > 0 ? (
            filteredProjects.map((p) => {
              ...
              return (
                <div
                  ...
                  className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm hover:shadow-xl hover:border-blue-400 hover:-translate-y-1.5 transition-all group relative overflow-hidden flex flex-col cursor-pointer min-h-[220px]"
                >
                  <div
                    className={`absolute top-0 left-0 w-full h-1.5 transition-colors ${
                      isRunning ? 'bg-blue-500' : 'bg-rose-500'
                    }`}
                  ></div>

                  <div className="flex justify-between items-start mb-4 mt-2">
                    <span
                      className={`px-3 py-1.5 text-[9px] font-black uppercase tracking-widest rounded-lg shadow-sm border ${
                        isRunning
                          ? 'bg-blue-50 text-blue-600 border-blue-100'
                    ...
                    <button
                      ...
                      className="p-2 text-slate-300 hover:text-rose-500 bg-white rounded-xl hover:bg-rose-50 transition-colors opacity-0 group-hover:opacity-100 border border-slate-100 hover:border-rose-200 shadow-sm"
                      title="Hapus Kamar Proyek"
                    >
                      <Trash size={16} />
                    </button>
                  </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-100">
                    <div className="flex justify-between items-end mb-2">
                      ...
                      <span className="text-xl font-black text-slate-800">
                        {Number(actualProg).toFixed(0)}%
                      </span>
                    </div>
                    <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden shadow-inner">
                      <div
                        className={`h-full rounded-full transition-all duration-1000 ${
                          isRunning ? 'bg-blue-500' : 'bg-slate-300'
                        }`}
                        style={{ width: `${actualProg}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
            ...
            <div className="col-span-full text-center py-20 bg-white rounded-[32px] border border-dashed border-slate-300">
              ...
              <p className="text-sm font-medium text-slate-500">
                Belum ada kamar proyek atau tidak ditemukan pencarian.
              </p>
            </div>
          )}
        </div>
...
export default function App() {
  ...
  const [masterForm, setMasterForm] = useState({
    dinas: JSON.parse(JSON.stringify(INITIAL_DINAS_DATA)),
    kontraktor: { fields: initialKontraktorFields, personil: [] },
    konsultan: { fields: initialKonsultanFields, personil: [] },
  });
    ...
    setDailyReportForm((prev) => {
      ...
      if (['tanggalMulai', 'jamMulai', 'jamSelesai'].includes(field)) {
        ...
        if (shift.tanggalMulai && shift.jamMulai && shift.jamSelesai) {
          const startM =
            parseInt(shift.jamMulai.substring(0, 2)) * 60 +
            parseInt(shift.jamMulai.substring(3, 5));
          const endM =
            parseInt(shift.jamSelesai.substring(0, 2)) * 60 +
            parseInt(shift.jamSelesai.substring(3, 5));
        ...
        try {
          ...
          if (newFeeds && newFeeds.length > 0) {
            ...
            if (
              ...
              const isAlreadyExist = feeds.some(
                (f) => f.id === currentLatestId
              );

              if (!isAlreadyExist) {
                showMsg(
                  `🔔 Notifikasi: ${newFeeds[0].title} baru saja masuk!`,
                  'success'
                );
  ...
  const fetchProjectDetails = async (projectId) => {
    try {
      ...
      if (proj) {
        ...
        let tKe = '1',
          tPct = '0';
        if ((proj.termin_ke || '').toString().includes(',')) {
          [tKe, tPct] = proj.termin_ke.split(',');
        } else {
          tKe = proj.termin_ke || '1';
        }

        setEditProjectForm({
          status: proj.status || 'Running',
          pekerjaan: proj.pekerjaan || '',
          termin_ke: tKe,
          termin_persen: tPct,
          panjang_rencana: proj.panjang_rencana || '',
          lebar_rencana: proj.lebar_rencana || '',
          jenis_model: proj.jenis_model || '',
          item_utama_data: proj.item_utama_data || [],
          waktu_pelaksanaan: proj.waktu_pelaksanaan || '',
        });

        let mergedDinas = Array.isArray(proj.dinas_data) ? proj.dinas_data : [];
        if (mergedDinas.length === 0)
          mergedDinas = JSON.parse(JSON.stringify(INITIAL_DINAS_DATA));

        setMasterForm({
          dinas: mergedDinas,
          kontraktor: {
            fields: buildDynamicFields(
              proj.kontraktor_data,
              initialKontraktorFields
            ),
            personil: Array.isArray(proj.kontraktor_data?.personil)
              ? proj.kontraktor_data.personil
              : [],
          },
          konsultan: {
            fields: buildDynamicFields(
              proj.konsultan_data,
              initialKonsultanFields
            ),
            personil: Array.isArray(proj.konsultan_data?.personil)
              ? proj.konsultan_data.personil
              : [],
          },
        });
        ...
        if (Array.isArray(proj.s_curve_data)) {
          ...
          setSCurveForm({
            plan: proj.s_curve_data
              .map((d) => (d.r != null ? String(d.r).replace('.', ',') : '0'))
              .join(' - '),
            actual: proj.s_curve_data
              .map((d) => (d.a != null ? String(d.a).replace('.', ',') : ''))
              .filter(Boolean)
              .join(' - '),
          });
        } else {
        ...
        if (
          proj.doc_categories &&
          Array.isArray(proj.doc_categories) &&
          proj.doc_categories.length > 0
        )
          setDocCategories(proj.doc_categories);
        else
          setDocCategories([
            'Administrasi',
            'Kontrak',
  ...
  const handleUpdateProject = async (e) => {
    ...
    try {
      ...
      const cleanPlan = String(sCurveForm.plan || '')
        .replace(/\s+/g, '')
        .replace(/,/g, '.')
        .split('-')
        .filter(Boolean);
      const cleanActual = String(sCurveForm.actual || '')
        .replace(/\s+/g, '')
        .replace(/,/g, '.')
        .split('-')
        .filter(Boolean);
      ...
      for (let i = 0; i < Math.max(cleanPlan.length, cleanActual.length); i++) {
        let point = {
          n: 'M' + (i + 1),
          r: isNaN(parseFloat(cleanPlan[i])) ? 0 : parseFloat(cleanPlan[i]),
        };
        if (i < cleanActual.length) {
          const aVal = parseFloat(cleanActual[i]);
          point.a =
            aVal === 0 && lastValidA > 0 ? null : isNaN(aVal) ? null : aVal;
        } else point.a = null;
      ...
      if (
        editProjectForm.item_utama_data &&
        editProjectForm.item_utama_data.length > 0
      ) {
        let totalBobot = 0;
        let weightedProgress = 0;
        editProjectForm.item_utama_data.forEach((item) => {
          const bobot =
            parseFloat(String(item.bobot || '0').replace(',', '.')) || 0;
          const persen =
            parseFloat(String(item.persen || '0').replace(',', '.')) || 0;
          totalBobot += bobot;
      ...
      const autoStatus =
        editProjectForm.status === 'Running' || calculatedProgress > 0
          ? 'Running'
      ...
      const { error } = await supabaseClient
        ...
        .update({
          ...
          pekerjaan: editProjectForm.pekerjaan,
          termin_ke: `${editProjectForm.termin_ke},${editProjectForm.termin_persen}`,
          status: autoStatus,
          actual_progress: parseFloat(calculatedProgress.toFixed(2)),
          panjang_rencana: editProjectForm.panjang_rencana,
          lebar_rencana: editProjectForm.lebar_rencana,
          jenis_model: editProjectForm.jenis_model,
          item_utama_data: editProjectForm.item_utama_data,
          waktu_pelaksanaan: editProjectForm.waktu_pelaksanaan,
      ...
      if (error) throw error;
      showMsg('Data Proyek Diperbarui!', 'success');
  ...
  const handleSaveRAB = async (rabItems) => {
    ...
    setIsProcessing(true);
    try {
      const cleanedItems = (rabItems || []).filter(
        (item) =>
      ...
      const { error } = await supabaseClient
        .from('projects')
        .update({
          rab_data: cleanedItems,
          updated_at: new Date().toISOString(),
        })
        .eq('id', projectData.id);
      if (error) throw error;
      setProjectData((prev) => ({ ...prev, rab_data: cleanedItems }));
      showMsg('Data RAB Berhasil Disimpan!', 'success');
    } catch (err) {
      showMsg('Gagal update DB RAB: ' + err.message, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleSaveSchedule = async (scheduleItems) => {
    ...
    try {
      const { error } = await supabaseClient
        .from('projects')
        .update({
          schedule_data: scheduleItems,
          updated_at: new Date().toISOString(),
        })
        .eq('id', projectData.id);
      if (error) throw error;
      setProjectData((prev) => ({ ...prev, schedule_data: scheduleItems }));
      showMsg('Jadwal Berhasil Disimpan!', 'success');
    } catch (err) {
      showMsg('Gagal update jadwal: ' + err.message, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleUpdateStakeholder = async (e) => {
    ...
    try {
      const { error } = await supabaseClient
        .from('projects')
        .update({
          dinas_data: masterForm.dinas,
          kontraktor_data: masterForm.kontraktor,
          konsultan_data: masterForm.konsultan,
          updated_at: new Date().toISOString(),
        })
        .eq('id', projectData.id);
      if (error) throw error;
      showMsg('Data Kontrak Disimpan!', 'success');
      setIsEditingContract(false);
      fetchProjectDetails(projectData.id);
    } catch (e) {
      showMsg('Gagal menyimpan kontrak: ' + e.message, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleOpenReportModal = () => {
    ...
    let baseShifts = [
      {
        id: Date.now(),
        tanggalMulai: baseDate,
        jamMulai: '08:00',
        tanggalSelesai: baseDate,
        jamSelesai: '17:00',
      },
    ];
      ...
      if (Array.isArray(tpl)) {
        ...
        if (tpl.tenagaKerja) {
          ...
          if (Array.isArray(tpl.tenagaKerja)) {
            ...
            savedTk = Object.keys(tpl.tenagaKerja).map((k) => ({
              posisi: k,
              jumlah: tpl.tenagaKerja[k],
            }));
          }

          // MEMAKSA URUTAN SESUAI DEFAULT (INITIAL_TENAGA_KERJA)
          const enforcedOrderTk = JSON.parse(
            JSON.stringify(INITIAL_TENAGA_KERJA)
          ).map((defaultItem) => {
            const found = savedTk.find(
              (item) =>
                String(item.posisi).toLowerCase() ===
                String(defaultItem.posisi).toLowerCase()
            );
            return found
              ? { ...defaultItem, jumlah: found.jumlah }
              : defaultItem;
          });
          ...
          savedTk.forEach((savedItem) => {
            if (savedItem.posisi && savedItem.posisi.trim() !== '') {
              const exists = enforcedOrderTk.find(
                (item) =>
                  String(item.posisi).toLowerCase() ===
                  String(savedItem.posisi).toLowerCase()
              );
              if (!exists) {
        ...
        if (
          tpl.aktivitas &&
          Array.isArray(tpl.aktivitas) &&
          tpl.aktivitas.length > 0
        ) {
          baseAktivitas = tpl.aktivitas.map((item) => ({
            nama: item.nama || '',
            kemarin: '',
            hariIni: '',
            satuan: item.satuan || '',
        ...
        if (tpl.shifts && Array.isArray(tpl.shifts) && tpl.shifts.length > 0) {
          baseShifts = tpl.shifts.map((s, idx) => {
            const startM =
              parseInt(s.jamMulai?.substring(0, 2) || '08') * 60 +
              parseInt(s.jamMulai?.substring(3, 5) || '00');
            const endM =
              parseInt(s.jamSelesai?.substring(0, 2) || '17') * 60 +
              parseInt(s.jamSelesai?.substring(3, 5) || '00');
            const tglSelesaiObj = new Date(baseDate);
            if (endM < startM)
              tglSelesaiObj.setDate(tglSelesaiObj.getDate() + 1);
            return {
              id: Date.now() + idx,
              tanggalMulai: baseDate,
              jamMulai: s.jamMulai || '08:00',
              tanggalSelesai: tglSelesaiObj.toISOString().split('T')[0],
              jamSelesai: s.jamSelesai || '17:00',
            };
          });
        }
      }
    ...
    try {
      ...
      if (quickRepFiles && quickRepFiles.length > 0) {
        ...
        for (let i = 0; i < quickRepFiles.length; i++) {
          ...
          const fileName = `${Date.now()}_quick_${i}.${fileToUpload.name
            .split('.')
            .pop()}`;
          await supabaseClient.storage
            .from('project-media')
            .upload(`reports/${fileName}`, fileToUpload);
      ...
      const { error } = await supabaseClient.from('field_reports').insert([
        {
          ...
          is_problem:
            !!quickReportNote &&
            quickReportNote.toLowerCase().includes('kendala'),
        },
      ]);
    ...
    try {
      ...
      if (repFiles && repFiles.length > 0) {
        ...
        for (let i = 0; i < repFiles.length; i++) {
          ...
          const fileName = `${Date.now()}_report_${i}.${fileToUpload.name
            .split('.')
            .pop()}`;
          await supabaseClient.storage
            .from('project-media')
      ...
      const cuacaStr = Object.entries(dailyReportForm.cuaca)
        .map(([jam, kondisi]) => `${jam} : ${kondisi}`)
        .join('\n');

      // Filter aktivitas yang kosong dan sinkronkan string
      const aktivStr = dailyReportForm.aktivitas
        .filter((a) => a.nama && (a.kemarin !== '' || a.hariIni !== ''))
        .map((a, i) => {
          const tot =
            (parseFloat(a.kemarin) || 0) + (parseFloat(a.hariIni) || 0);
          return `- ${a.nama} : Kemarin=${a.kemarin || 0}, Hari Ini=${
            a.hariIni || 0
          }, Total=${tot} ${a.satuan}`;
        })
        .join('\n');
      ...
      const tkStr = (
        Array.isArray(dailyReportForm.tenagaKerja)
          ? dailyReportForm.tenagaKerja
          : []
      )
      ...
      if (dailyReportForm.shifts && dailyReportForm.shifts.length > 0) {
        waktuStr = dailyReportForm.shifts
          .map((s, i) => {
            const shiftTitle =
              dailyReportForm.shifts.length > 1 ? `Shift ${i + 1}: ` : '';
            if (s.tanggalMulai === s.tanggalSelesai) {
      ...
      const reportContent = `📋 LAPORAN HARIAN\nTanggal: ${
        dailyReportForm.tanggal
      }\nLokasi: ${
        dailyReportForm.lokasi || '-'
      }\nWaktu:\n${waktuStr}\n\n🌤️ KONDISI CUACA:\n${cuacaStr}\n\n👷 TENAGA KERJA:\n${
        tkStr || '-'
      }\n\n🚧 AKTIVITAS PEKERJAAN:\n${
        aktivStr || '-'
      ...
      const isProblem =
        !!dailyReportForm.catatan &&
        dailyReportForm.catatan.toLowerCase().includes('kendala');
      ...
      dailyReportForm.aktivitas.forEach((lapAct) => {
        ...
        const itemIdx = updatedItemUtama.findIndex(
          (u) => String(u.nama).trim().toLowerCase() === targetName
        );
        ...
        if (itemIdx !== -1) {
          ...
          if (lapAct.hariIni !== '' || lapAct.kemarin !== '') {
            ...
            updatedItemUtama[itemIdx].persen = parseFloat(
      ...
      if (isItemUtamaChanged) {
        ...
        await supabaseClient
          .from('projects')
          .update({ item_utama_data: updatedItemUtama })
          .eq('id', projectData.id);
      }
      // ---------------------------------------------------------------
      ...
      const newTemplate = {
        ...
        shifts: dailyReportForm.shifts.map((s) => ({
          jamMulai: s.jamMulai,
          jamSelesai: s.jamSelesai,
        })),
        aktivitas: dailyReportForm.aktivitas
          .filter((a) => String(a.nama || '').trim() !== '')
          .map((a) => ({ nama: a.nama, satuan: a.satuan })),
        tenagaKerja: dailyReportForm.tenagaKerja.filter(
          (tk) => tk.posisi && tk.posisi.trim() !== ''
        ),
      };
      ...
      await supabaseClient
        .from('projects')
        .update({ report_template_data: newTemplate })
        .eq('id', projectData.id);
      setProjectData((prev) => ({
        ...prev,
        report_template_data: newTemplate,
      })); // Update local state
      // ---------------------------------------------

      showMsg('Laporan Harian Terkirim! Mengunduh bukti PDF...', 'success');
      if (window.html2canvas) {
        await generateDailyReportReceipt(dailyReportForm, projectData);
      }
      ...
      setDailyReportForm({
        ...
        shifts: [
          {
            id: Date.now(),
            tanggalMulai: resetDate,
            jamMulai: '08:00',
            tanggalSelesai: resetDate,
            jamSelesai: '17:00',
          },
        ],
        cuaca: {
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      showMsg(
        'Template Form Laporan (Lokasi, Shift, Aktivitas & Naker) Berhasil Disimpan!',
        'success'
      );
    } catch (e) {
  ...
  const handleUnifiedSubmit = async (e) => {
    ...
    try {
      ...
      const sLat_in = parseCoord(uForm.startLat);
      const sLng_in = parseCoord(uForm.startLng);
      const eLat_in = parseCoord(uForm.endLat);
      const eLng_in = parseCoord(uForm.endLng);
      ...
      let newSegments = Array.isArray(projectData.actual_segments_data)
        ? [...projectData.actual_segments_data]
        : [];
      ...
      if (segName) {
        existingSegIdx = newSegments.findIndex(
          (s) => String(s.name).toLowerCase() === segName.toLowerCase()
        );
      } else {
        let count = 1;
        while (
          newSegments.find(
            (s) => String(s.name).toLowerCase() === `segmen ${count}`
          )
        ) {
          count++;
        }
        segName = `Segmen ${count}`;
      }

      const isMasterCoordEmpty =
        !projectData.start_lat || projectData.start_lat === '-';
      ...
      const isPathEmpty =
        ...
        (currentPath.length === 1 && currentPath[0].length === 0);
      if (
        sLat_in !== null &&
        sLng_in !== null &&
        eLat_in !== null &&
        ...
        updatePayload.planned_path = [
          [
            { lat: sLat_in, lng: sLng_in, sta: 'Awal' },
            { lat: eLat_in, lng: eLng_in, sta: 'Akhir' },
          ],
        ];
      }
      ...
      if (uMedia && uMedia.length > 0) {
        ...
        const uploadPromises = uMedia.map(async (file, index) => {
          ...
          const fileName = `${Date.now()}_${index}_gallery.${fileToUpload.name
            .split('.')
            .pop()}`;
          await supabaseClient.storage
            .from('project-media')
          ...
          return {
            project_id: projectData.id,
            title: `Dokumentasi Survei 0% (Part ${index + 1})`,
            description: uForm.noteDesc || 'Dokumentasi awal',
          ...
          .insert([
            {
              ...
              description: `Tanggal Survei: ${uForm.tanggal}\n\n${uForm.noteDesc}`,
              media_url: null,
              is_problem: false,
            },
          ]);
      ...
      if (uDataUkur) {
        const fileName = `${Date.now()}_data_ukur.${uDataUkur.name
          .split('.')
          .pop()}`;
        await supabaseClient.storage
          .from('project-media')
          .upload(`documents/${fileName}`, uDataUkur);
        const { data: urlData } = supabaseClient.storage
          .from('project-media')
          ...
          .insert([
            {
              ...
              name: uDataUkur.name,
              category: 'Survei',
              size: (uDataUkur.size / 1024 / 1024).toFixed(2) + ' MB',
      ...
      await supabaseClient.from('field_reports').insert([
        {
          ...
          description: `Tim lapangan telah mengirimkan data hasil survei untuk ${segName}.\nPanjang Eks.: ${
            uForm.panjang || '-'
          }m | Lebar Eks.: ${uForm.lebar || '-'}m | Model Eks.: ${
            uForm.jenis_model_awal || '-'
          }\nKoordinat: Awal(${sLat_in !== null ? sLat_in : '-'}, ${
            sLng_in !== null ? sLng_in : '-'
          }) Akhir(${eLat_in !== null ? eLat_in : '-'}, ${
            eLng_in !== null ? eLng_in : '-'
          })`,
        },
      ]);
      ...
      showMsg('Data Survei Tersimpan!', 'success');
      ...
      showMsg('Error menyimpan survei: ' + err.message, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const getUnifiedGPS = (type) => {
    if (!navigator.geolocation) {
      showMsg('GPS tidak didukung browser ini.', 'error');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        showMsg(
          `GPS Titik ${type === 'start' ? 'Awal' : 'Akhir'} Ditemukan!`,
          'success'
        );
      },
      (err) => {
  ...
  const handleRoutesUpdate = async (planned, actualSegs) => {
    ...
    try {
      const payload = {
        ...
        actual_segments_data: actualSegs,
        updated_at: new Date().toISOString(),
      };
      if (actualSegs && actualSegs.length > 0) {
        const fSeg = actualSegs[0];
        if (fSeg.startLat !== '' && fSeg.startLng !== '') {
          payload.start_lat = parseFloat(fSeg.startLat);
          payload.start_lng = parseFloat(fSeg.startLng);
        }
        if (fSeg.endLat !== '' && fSeg.endLng !== '') {
          payload.end_lat = parseFloat(fSeg.endLat);
          payload.end_lng = parseFloat(fSeg.endLng);
        }
      }
      const { error } = await supabaseClient
        .from('projects')
      ...
      showMsg('Gagal sinkronisasi rute: ' + e.message, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleDocUpload = async (e) => {
    e.preventDefault();
    if (!docFile) {
      showMsg('Pilih file terlebih dahulu', 'error');
      return;
    }
    if (!supabaseClient || !projectData) return;
    setIsProcessing(true);
    try {
      const fileName = `${Date.now()}_${docFile.name}`;
      await supabaseClient.storage
        .from('project-media')
        .upload(`documents/${fileName}`, docFile);
      const { data: urlData } = supabaseClient.storage
        .from('project-media')
        .getPublicUrl(`documents/${fileName}`);
      const { error } = await supabaseClient
      ...
      if (error) throw error;
      showMsg('Dokumen Tersimpan!', 'success');
      setShowDocModal(false);
      setDocFile(null);
      fetchProjectDetails(projectData.id);
    } catch (e) {
      showMsg('Gagal upload dokumen: ' + e.message, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleDropReportUpload = async (e) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (
        ...
        handleDirectReportUpload({ target: { files: [file] } });
      } else {
        showMsg('Hanya menerima file berformat PDF!', 'error');
      }
    }
  };
  ...
  const handleDirectReportUpload = async (e) => {
    const file = e.target.files[0];
    if (!file || !supabaseClient || !projectData) return;
    setIsProcessing(true);
    try {
      ...
      if (error) throw error;
      showMsg('Laporan Mingguan PDF Tersimpan!', 'success');
      fetchProjectDetails(projectData.id);
    } catch (err) {
      showMsg('Gagal upload PDF: ' + err.message, 'error');
    } finally {
      setIsProcessing(false);
      if (reportFileInputRef.current) reportFileInputRef.current.value = null;
    }
  };
  ...
  const confirmDeleteData = async () => {
    ...
    try {
      ...
      if (deletedType === 'media')
        setFeeds((prev) => prev.filter((f) => f.id !== deletedId));
      else if (deletedType === 'doc')
  ...
  const handleSaveEmployee = async (e) => {
    ...
    try {
      ...
      showMsg('Gagal menyimpan karyawan: ' + err.message, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleMoveDocument = async () => {
    if (!moveDocConfig) return;
    setIsProcessing(true);
    try {
      const newCat = moveDocConfig.newCategory || moveDocConfig.category;
      if (newCat === moveDocConfig.category) {
        setMoveDocConfig(null);
        setIsProcessing(false);
        return;
      }
      const { error } = await supabaseClient
        ...
        .eq('id', moveDocConfig.id);
      if (error) throw error;
      showMsg('Kategori diubah!', 'success');
      setMoveDocConfig(null);
      fetchProjectDetails(projectData.id);
    } catch (err) {
      showMsg(err.message, 'error');
  ...
  const handleSCurveTextSubmit = async (e) => {
    ...
    try {
      ...
      showMsg('Error menyimpan Kurva S: ' + err.message, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleDragOver = (e, index) => {
    ...
    const newCats = [...docCategories];
    const draggedCat = newCats[draggedIdx];
  ...
  const handleDragEnd = () => {
    setDraggedIdx(null);
    if (projectData && supabaseClient) {
      supabaseClient
        .from('projects')
        .update({ doc_categories: docCategories })
        .eq('id', projectData.id)
        .then();
    }
  };
          ...
          if (['mp4', 'webm', 'ogg', 'mov'].includes(ext)) {
            return (
              <video
                ...
                src={u}
                controls
                className="w-full h-auto max-h-[60vh] object-contain rounded-2xl bg-black"
              />
            );
          } else if (ext === 'pdf') {
            return (
            ...
            return (
              <img
                key={i}
                src={u}
                alt={`Lampiran Media ${i + 1}`}
                className="w-full h-auto max-h-[60vh] object-contain rounded-2xl bg-slate-100"
              />
            );
          }
        })}
  ...
  const surveyLogs = useMemo(
    () =>
      safeFeeds
        .filter((f) => f.title === 'Pengiriman Data Survei')
        .sort(
          (a, b) =>
  ...
  useEffect(() => {
    if (surveyLogs.length > 0) {
      if (
        !activeSurveyLogId ||
        !surveyLogs.find((l) => l.id === activeSurveyLogId)
      ) {
        setActiveSurveyLogId(surveyLogs[0].id);
      }
    } else {
      setActiveSurveyLogId('');
    }
  ...
  const {
    ...
    let act = null,
      ...
      pA = 0,
      week = '-';
    const proc = (sCurveData || []).map((d) => {
      let a = parseFloat(d.a);
      if (isNaN(a) || (a === 0 && pA > 0)) a = null;
      if (a !== null) pA = a;
      return { ...d, a };
    });
    if (proc.length > 0) {
      const aVals = proc.map((d) => d.a);
      let lastIdx = -1;
    ...
    const currentActual = act !== null ? act : 0;
    const dev = (currentActual - tgt).toFixed(2);
    const isDevPos = currentActual >= tgt;
    return {
      actualProg: act,
      lastUpdatedWeek: week,
      deviasi: dev,
      isDeviasiPositive: isDevPos,
      processedSCurveData: proc,
    };
  }, [sCurveData]);
  ...
  const { terminNum, terminPct } = useMemo(() => {
    let tN = '1',
      tP = '0';
    if ((projectData?.termin_ke || '').includes(','))
      [tN, tP] = projectData.termin_ke.split(',');
  ...
  const sisaWaktuInfo = useMemo(() => {
    const spmk =
      findDynamicValue(
        projectData?.kontraktor_data,
        ['tanggal spmk', 'tgl spmk'],
        ['tanggal_spmk', 'tgl_spmk']
      ) || projectData?.tanggal_spmk;
    const waktu =
      projectData?.waktu_pelaksanaan ||
      findDynamicValue(
        projectData?.kontraktor_data,
        ['waktu pelaksanaan'],
        ['waktu_pelaksanaan']
    ...
    if (!spmk && !waktu)
      return { value: '-', sub: 'Data belum lengkap', status: 'neutral' };

    let durationDays = 0;
    if (waktu) {
      durationDays = parseInt(String(waktu).replace(/[^0-9]/g, ''), 10);
    }

    if (!spmk && waktu) {
      if (isNaN(durationDays) || durationDays === 0)
        return { value: '-', sub: 'Format waktu salah', status: 'neutral' };
      return {
        value: `${durationDays}\nHari`,
        sub: `Total Waktu Pelaksanaan: ${durationDays} Hari\nSPMK Awal: Belum diatur\nSPMK Akhir: Belum diatur`,
        status: 'neutral',
      };
    }
    if (spmk && !waktu)
      return {
        value: '-',
        sub: 'Waktu Pelaksanaan belum diatur',
        status: 'neutral',
      };

    const spmkDate = new Date(spmk);
    if (isNaN(durationDays) || isNaN(spmkDate.getTime()))
      return { value: '-', sub: 'Format waktu salah', status: 'neutral' };

    const endDate = new Date(spmkDate);
    endDate.setDate(spmkDate.getDate() + (durationDays - 1));
    const diffDays = Math.ceil((endDate - new Date()) / (1000 * 60 * 60 * 24));

    const subText = `Total Waktu Pelaksanaan: ${durationDays} Hari\nSPMK Awal: ${spmkDate.toLocaleDateString(
      'id-ID'
  ...
  const currentPhase =
    dashboardMode === 'auto'
      ? !!findDynamicValue(
          projectData?.kontraktor_data,
          ['nomor kontrak', 'no kontrak'],
          ['nomor_kontrak']
        ) ||
        parseFloat(projectData?.actual_progress || 0) > 0 ||
  ...
  const filteredDocs = useMemo(
    () =>
      safeDocuments.filter(
        (d) =>
          (docFilterCategory === 'Semua' || d.category === docFilterCategory) &&
          String(d.name || '')
            .toLowerCase()
            .includes(docSearchQuery.toLowerCase())
      ),
    [safeDocuments, docFilterCategory, docSearchQuery]
  );
  const photos = useMemo(() => {
    let allPhotos = [];
    safeFeeds
  ...
  const persiapanSidebarFeeds = useMemo(
    () =>
      safeFeeds.filter(
        (f) =>
          !String(f.title || '').includes('Dokumentasi Survei 0%') &&
          f.title !== 'Catatan Survei'
      ),
    [safeFeeds]
  );
  ...
  const dashboardData = useMemo(() => {
    ...
    if (activeLog) {
      tglStr =
        new Date(activeLog.created_at).toLocaleDateString('id-ID', {
          day: '2-digit',
          month: 'long',
          year: 'numeric',
        }) +
        ' ' +
        new Date(activeLog.created_at).toLocaleTimeString('id-ID', {
          hour: '2-digit',
          minute: '2-digit',
        }) +
        ' WIB';

      const matchSeg = String(activeLog.description || '').match(
        /data hasil survei untuk (.*?)\.\n/
      );
      const matchPanjang = String(activeLog.description || '').match(
        /(?:Panjang|Panjang Eks\.)\s*:\s*(.*?)m\s*\|/
      );
      const matchLebar = String(activeLog.description || '').match(
        /(?:Lebar|Lebar Eks\.)\s*:\s*(.*?)m\s*\|/
      );
      const matchModel = String(activeLog.description || '').match(
        /(?:Model|Model Eks\.)\s*:\s*(.*?)(?:\n|$)/
      );

      if (matchSeg && matchSeg[1]) displaySegName = matchSeg[1].trim();
      if (matchPanjang && matchPanjang[1])
        displayPanjang = matchPanjang[1].trim();
      if (matchLebar && matchLebar[1]) displayLebar = matchLebar[1].trim();
      if (matchModel && matchModel[1]) displayModel = matchModel[1].trim();

      const matchCoordLog = String(activeLog.description || '').match(
        /Koordinat:\s*Awal\(([^,]+),\s*([^)]+)\)\s*Akhir\(([^,]+),\s*([^)]+)\)/
      );
      if (matchCoordLog) {
        sLat = matchCoordLog[1].trim();
        sLng = matchCoordLog[2].trim();
        eLat = matchCoordLog[3].trim();
        eLng = matchCoordLog[4].trim();
      }

      const T = new Date(activeLog.created_at).getTime();
      const timeWindow = 15000;

      let rawPhotos = safeFeeds.filter(
        (f) =>
          String(f.title || '').includes('Dokumentasi Survei') &&
          Math.abs(new Date(f.created_at).getTime() - T) <= timeWindow
      );
      ...
      displayNote = safeFeeds.find(
        (n) =>
          String(n.title || '').includes('Catatan Survei') &&
          Math.abs(new Date(n.created_at).getTime() - T) <= timeWindow
      );
      displayCsv = safeDocuments.find(
        (d) =>
          String(d.name || '')
            .toLowerCase()
            .endsWith('.csv') &&
          Math.abs(new Date(d.created_at).getTime() - T) <= timeWindow
      );
    }
  ...
  const contractDataArrays = useMemo(() => {
    let dData =
      Array.isArray(projectData?.dinas_data) &&
      projectData.dinas_data.length > 0
        ? projectData.dinas_data.map((d) => ({
            label: String(d.role || 'Instansi'),
            value: String(d.name || ''),
            sub: d.nip ? `NIP: ${String(d.nip)}` : '',
          }))
        : INITIAL_DINAS_DATA.map((d) => ({
            label: String(d.role || 'Instansi'),
            value: String(d.name || ''),
            sub: d.nip ? `NIP: ${String(d.nip)}` : '',
          }));
    ...
    const kData = (
      projectData?.kontraktor_data?.fields ||
      buildDynamicFields(projectData?.kontraktor_data, initialKontraktorFields)
    ).map((f) => ({
      label: String(f.label || ''),
      value: String(f.value || ''),
    }));
    (Array.isArray(projectData?.kontraktor_data?.personil)
      ? projectData.kontraktor_data.personil
      : []
    ).forEach((p, i) => {
      kData.push({
        label: `${String(p.name || `Nama Personil ${i + 1}`)}`,
        value: String(p.position || 'Jabatan'),
      });
    });

    const cData = (
      projectData?.konsultan_data?.fields ||
      buildDynamicFields(projectData?.konsultan_data, initialKonsultanFields)
    ).map((f) => ({
      label: String(f.label || ''),
      value: String(f.value || ''),
    }));
    (Array.isArray(projectData?.konsultan_data?.personil)
      ? projectData.konsultan_data.personil
      : []
    ).forEach((p, i) => {
      cData.push({
        label: `${String(p.name || `Nama Personil ${i + 1}`)}`,
  ...
  if (appMode === 'selection') {
    return (
      <>
        {notification && (
          <div
            className={`fixed top-10 left-1/2 -translate-x-1/2 z-[6000] px-6 py-4 rounded-2xl shadow-xl border backdrop-blur-xl animate-in slide-in-from-top-5 flex items-center gap-3 ${
            ...
            <div className="bg-white rounded-[32px] p-8 w-full max-w-md shadow-2xl relative">
              ...
              <form onSubmit={handleCreateProject} className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full p-3.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-bold text-slate-800 outline-none focus:border-blue-400 focus:bg-white transition-all"
                    value={newProjectForm.pekerjaan}
                    onChange={(e) =>
                      setNewProjectForm({
                        ...newProjectForm,
                    ...
                    placeholder="Misal: Peningkatan Jalan Baru..."
                    required
                  />
                </div>
                <div>
                  ...
                  <select
                    className="w-full p-3.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-bold text-slate-700 outline-none focus:border-blue-400 focus:bg-white transition-all"
                ...
                <button
                  type="submit"
                  disabled={isProcessing}
                  className="w-full bg-blue-600 text-white py-4 rounded-2xl text-xs font-bold uppercase mt-4 hover:bg-blue-700 transition-colors shadow-md"
                >
                  {isProcessing ? (
                    <Loader2 size={16} className="animate-spin mx-auto" />
                  ) : (
        ...
        {deleteConfig && deleteConfig.type === 'project' && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[6000] p-4">
            <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-sm shadow-2xl relative text-center">
              ...
              <p className="text-xs text-rose-600 mb-8 font-medium leading-relaxed bg-rose-50 p-3 rounded-xl border border-rose-100">
                Kamar proyek <b>{deleteConfig.name}</b> dan seluruh data
                didalamnya akan dihapus permanen!
              </p>
              <div className="flex gap-3">
                <button
  ...
  if (appMode === 'absensi') {
    return (
      <>
        {notification && (
          <div
            className={`fixed top-10 left-1/2 -translate-x-1/2 z-[8000] px-6 py-4 rounded-2xl shadow-xl border backdrop-blur-xl animate-in slide-in-from-top-5 pointer-events-none flex items-center gap-3 ${
              notification.type === 'error'
                ? 'bg-rose-100 text-rose-700 border-rose-200'
                : 'bg-emerald-50 text-emerald-600 border-emerald-100'
            }`}
        ...
        {showEmployeeModal && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[9000] p-4">
            <div className="bg-white rounded-[32px] p-8 w-full max-w-md shadow-2xl relative">
              ...
              <h3 className="text-lg font-black mb-6 uppercase text-slate-800 tracking-tight">
                {employeeForm.id
                  ? 'Edit Data Karyawan'
              ...
              <form onSubmit={handleSaveEmployee} className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold block mb-1.5 uppercase text-slate-500">
                    ID Karyawan (Unik) <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="text"
                    className="w-full p-3.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-bold text-slate-800 outline-none focus:border-blue-400 focus:bg-white transition-all"
                ...
                <div>
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    pattern="[0-9]*"
                    className="w-full p-3.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-mono font-bold text-slate-800 outline-none focus:border-blue-400 focus:bg-white transition-all"
                    ...
                    required
                    title="Hanya gunakan karakter Angka"
                  />
                </div>
                <button
                  type="submit"
                  disabled={isProcessing}
                  className="w-full bg-blue-600 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest mt-4 hover:bg-blue-700 transition-colors shadow-md"
                >
                  {isProcessing ? (
                    <Loader2 size={16} className="animate-spin mx-auto" />
                  ) : employeeForm.id ? (
        ...
        {deleteConfig && deleteConfig.type === 'employee' && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[9000] p-4">
            <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-sm shadow-2xl relative text-center">
              ...
              <p className="text-xs text-slate-500 mb-8 font-medium leading-relaxed">
                Data karyawan atas nama <b>{deleteConfig.name}</b> akan dihapus
                secara permanen.
              </p>
              <div className="flex gap-3">
                <button
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#f4f7fe] text-slate-800 overflow-hidden font-sans">
      ...
      <aside
        className={`${
          isSidebarOpen ? 'hidden md:flex w-56' : 'hidden'
        } border-r border-slate-200 flex-col bg-white shrink-0 shadow-sm relative z-[100] pointer-events-auto transition-all duration-300`}
      >
        <div className="p-5">
          ...
          <div className="flex items-center justify-between mb-8 px-1">
            <div className="flex items-center gap-2.5">
              ...
              <h1 className="text-lg font-black tracking-tight text-slate-800">
                Synx<span className="text-blue-500">Build</span>
              </h1>
            </div>
            <button
              onClick={() => setIsSidebarOpen(false)}
              className="p-1.5 text-slate-400 hover:text-slate-700 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors border border-slate-200 shadow-sm"
          ...
          <nav className="space-y-1.5">
            ...
            >
              <Briefcase size={18} /> Data Kontrak
            </button>
            <button
              onClick={() => setActiveMenu('dokumentasi')}
              className={`w-full flex items-center gap-3 px-3.5 py-3.5 rounded-xl text-xs font-bold transition-all ${
            ...
            >
              <FolderEdit size={18} /> Arsip Dokumen
            </button>
            <button
              onClick={() => setActiveMenu('rab')}
              className={`w-full flex items-center gap-3 px-3.5 py-3.5 rounded-xl text-xs font-bold transition-all ${
                activeMenu === 'rab'
                  ? 'bg-blue-600 text-white shadow-md'
                  : 'text-slate-500 hover:bg-slate-50'
            ...
            >
              <Calculator size={18} /> RAB / BOQ
            </button>
            <button
              onClick={() => setActiveMenu('presentation')}
              className={`w-full flex items-center gap-3 px-3.5 py-3.5 rounded-xl text-xs font-bold transition-all ${
            ...
            >
              <MonitorPlay size={18} /> Presentasi
            </button>
          </nav>
        </div>
        <div className="mt-auto p-5 border-t border-slate-100 flex flex-col gap-3">
          ...
          <div className="flex bg-slate-50 p-1 rounded-xl">
            <button
              ...
              className={`flex-1 py-2 flex justify-center rounded-lg transition-all ${
                themeSetting === 'light'
          ...
          <button
            ...
            className="w-full bg-slate-50 py-3 rounded-xl text-[9px] font-bold uppercase flex justify-center items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors"
          >
            <Database size={14} /> DB Setup
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-hidden flex flex-col relative bg-[#f4f7fe] pb-20 md:pb-0 h-full">
        {activeMenu === 'dashboard' && (
          <div className="h-full flex flex-col">
              ...
              <div className="flex items-start gap-2 md:gap-3 w-full">
                ...
                <div className="md:hidden mt-0.5 shrink-0">
                  <button
                    type="button"
                    onClick={handleBackFromProject}
                    className="p-2 bg-white border border-slate-200 rounded-xl text-slate-500 hover:bg-slate-50 hover:text-blue-600 transition-colors cursor-pointer block relative z-[99999] pointer-events-auto"
                    title="Kembali ke Menu Utama"
                  >
                    <ArrowLeft size={18} />
                  </button>
                ...
                <div className="min-w-0 flex-1 pr-2">
                  <h2
                    className="text-sm md:text-base lg:text-lg font-black uppercase tracking-tight text-slate-800 leading-snug line-clamp-2 w-full pointer-events-auto"
                    title={String(
                      projectData?.pekerjaan || 'JUDUL PEKERJAAN KONTRAK'
                    )}
                  >
                    {String(
                      projectData?.pekerjaan || 'JUDUL PEKERJAAN KONTRAK'
              ...
              <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-3 w-full">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-3">
                  ...
                  <div className="flex gap-1 p-1 bg-slate-100/80 w-max rounded-lg border border-slate-200/60 shadow-inner shrink-0">
                    <button
                      onClick={() => setDashboardMode('persiapan')}
                      className={`px-4 py-2 text-[9px] md:text-[10px] font-black uppercase tracking-wider rounded-md transition-all ${
                        currentPhase === 'persiapan'
                    ...
                    <button
                      onClick={() => setDashboardMode('pelaksanaan')}
                      className={`px-4 py-2 text-[9px] md:text-[10px] font-black uppercase tracking-wider rounded-md transition-all ${
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="flex items-center gap-1.5 bg-white border border-slate-200 px-3 py-2 rounded-lg shadow-sm text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                      <Ruler size={14} className="text-blue-500" /> Panjang:{' '}
                      <span className="text-slate-800 font-black">
                        {projectData?.panjang_rencana
                          ? `${projectData.panjang_rencana} m`
                          : '-'}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 bg-white border border-slate-200 px-3 py-2 rounded-lg shadow-sm text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                      <Ruler size={14} className="rotate-90 text-amber-500" />{' '}
                      Lebar:{' '}
                      <span className="text-slate-800 font-black">
                        {projectData?.lebar_rencana
                          ? `${projectData.lebar_rencana} m`
                          : '-'}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 bg-white border border-slate-200 px-3 py-2 rounded-lg shadow-sm text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                      ...
                      />{' '}
                      <span className="shrink-0">Saluran:</span>{' '}
                      <span
                        className="text-slate-800 font-black truncate max-w-[120px]"
                        title={projectData?.jenis_model || '-'}
                      >
                        {projectData?.jenis_model || '-'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0 relative z-[99999] w-full xl:w-auto mt-2 xl:mt-0">
                  <button
                    type="button"
                    onClick={() =>
                      setReadFeeds(new Set(safeFeeds.map((f) => f.id)))
                    }
                    className="relative z-50 p-2 text-slate-400 hover:text-blue-600 transition-colors bg-white hover:bg-blue-50 rounded-xl shadow-sm border border-slate-200 cursor-pointer pointer-events-auto"
                    title="Tandai semua log sudah dibaca"
                  >
                    <Bell size={16} />
                    {safeFeeds.filter((f) => !readFeeds.has(f.id)).length >
                      0 && (
                      <span className="absolute -top-1.5 -right-1.5 flex items-center justify-center min-w-[16px] h-[16px] px-1 rounded-full bg-rose-500 text-white text-[8px] font-black border border-white shadow-sm animate-pulse pointer-events-none">
                        ...
                        99
                          ? '99+'
                          : safeFeeds.filter((f) => !readFeeds.has(f.id))
                              .length}
                      </span>
                    )}
                  </button>
            ...
            <div className="p-4 md:p-6 lg:p-8 flex-1 overflow-y-auto no-scrollbar relative z-10">
              {currentPhase === 'persiapan' ? (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 xl:gap-8 items-start animate-in fade-in zoom-in-95 duration-500">
                  {/* KIRI: DATA SURVEI (TABEL) */}
                  <div className="lg:col-span-9 flex flex-col">
                    <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm w-full h-auto flex flex-col overflow-hidden">
                      <div className="p-6 md:p-8 bg-[#f8fafc] border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div className="flex items-center gap-4">
                          <div className="p-3 bg-amber-100 text-amber-600 rounded-2xl shrink-0">
                            <HardHat size={24} />
                          </div>
                          <div>
                            <h3 className="text-sm md:text-base font-black text-slate-800 uppercase tracking-widest">
                              Data Survei
                            </h3>
                            <p className="text-[10px] text-slate-500 font-bold uppercase mt-0.5 tracking-wider">
                              Data Kondisi Eksisting Lapangan
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          {surveyLogs.length > 0 && (
                            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-sm">
                              ...
                              />
                              <select
                                className="text-[10px] font-bold text-slate-700 bg-transparent outline-none cursor-pointer w-auto"
                                value={activeSurveyLogId}
                                onChange={(e) =>
                                  setActiveSurveyLogId(e.target.value)
                                }
                              >
                                {surveyLogs.map((log) => {
                                  const d = new Date(log.created_at);
                                  const dtStr = d.toLocaleDateString('id-ID', {
                                    day: '2-digit',
                                    month: 'short',
                                    year: 'numeric',
                                  });
                      ...
                      <div className="flex flex-col">
                        ...
                        />
                        <SurveyDetailRow
                          label="Nama Jln/Gg./Blok"
                          value={safeRender(displaySegName)}
                          isEven={true}
                        />
                        <SurveyDetailRow
                          label="Titik Awal"
                          value={
                            safeRender(sLat) !== '-' && safeRender(sLng) !== '-'
                              ? `${sLat}, ${sLng}`
                              : '-'
                          }
                          isEven={false}
                        ...
                        <SurveyDetailRow
                          label="Titik Akhir"
                          value={
                            safeRender(eLat) !== '-' && safeRender(eLng) !== '-'
                              ? `${eLat}, ${eLng}`
                              : '-'
                          }
                          isEven={true}
                        ...
                        />
                        <SurveyDetailRow
                          label="Jenis/Model Eks."
                          value={safeRender(displayModel)}
                          isEven={false}
                        />
                        <SurveyDetailRow
                          label="Upload Data Ukur (CSV)"
                          value={
                            displayCsv && displayCsv.file_url ? (
                              <a
                                href={displayCsv.file_url}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-blue-600 hover:underline flex items-center gap-1 w-fit"
                              >
                                <FileSpreadsheet size={14} />{' '}
                                {String(displayCsv.name || '')}
                              </a>
                            ) : (
                              '-'
                            )
                        ...
                        />
                        <SurveyDetailRow
                          label="Catatan - Kendala - Kondisi"
                          value={safeRender(noteText)}
                          isEven={false}
                        />
                      </div>
                      ...
                      <div className="p-6 md:p-8 bg-slate-50 border-t border-slate-100">
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">
                          Dokumentasi Eksisting (0%)
                        </p>
                        {displayPhotos.length > 0 ? (
                          <div className="flex overflow-x-auto gap-4 custom-scrollbar pb-4 snap-x">
                            {displayPhotos.map((photo, idx) => (
                              <div
                                key={photo.display_id || idx}
                                className="min-w-[160px] w-[160px] aspect-square rounded-2xl overflow-hidden border border-slate-200 cursor-pointer shadow-sm hover:shadow-md transition-all relative group snap-start"
                                onClick={() => setSelectedLog(photo)}
                              >
                                {isVideo(photo.media_url) ? (
                                  <video
                                    src={photo.media_url}
                                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                                  />
                  ...
                  <div className="lg:col-span-3 flex flex-col">
                    <div className="bg-white p-5 md:p-6 lg:p-8 rounded-[32px] border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.03)] flex flex-col h-[450px] shrink-0">
                      <h3 className="text-[9px] lg:text-[10px] font-black text-slate-400 mb-2 tracking-[0.3em] uppercase flex justify-between shrink-0">
                        Aktivitas Terkini{' '}
                        <div className="w-1.5 h-1.5 rounded-full bg-rose-400 animate-pulse" />
                      </h3>
                      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col divide-y divide-slate-100">
                        {safeFeeds.length > 0 ? (
                          safeFeeds
                            .slice(0, 10)
                            .map((f) => (
                              <FeedItem
                                ...
                                onDelete={() =>
                                  setDeleteConfig({ id: f.id, type: 'media' })
                                }
                                isUnread={!readFeeds.has(f.id)}
                              />
                            ))
                        ) : (
                          <div className="text-center py-12 flex flex-col items-center opacity-50 m-auto">
                            <Activity
                              size={32}
                      ...
                      <button
                        onClick={() => setActiveMenu('dokumentasi')}
                        className="w-full mt-4 py-3 bg-slate-50 text-slate-600 rounded-xl text-xs font-bold border border-slate-100 hover:bg-slate-100 hover:text-blue-600 transition-colors shrink-0"
                      >
                        Lihat Semua Riwayat
                      </button>
                    </div>
                ...
                <div className="animate-in fade-in zoom-in-95 duration-500 w-full flex flex-col pb-10">
                  ...
                  <div className="flex flex-col gap-6 mb-8">
                    ...
                    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-4 md:gap-6 lg:h-[300px]">
                      ...
                      <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 lg:h-full">
                        <CircularStatCard
                          ...
                          trend={true}
                          isPositive={isDeviasiPositive}
                          dropShadowColor="emerald"
                          subContent={
                            <div className="flex flex-col items-center mt-1">
                              <span className="text-[12px] md:text-sm font-bold uppercase tracking-wider text-slate-800">
                                {String(lastUpdatedWeek)
                                  .replace('M', 'Minggu Ke-')
                                  .replace('W', 'Minggu Ke-')}
                              </span>
                              <div className="text-[10px] font-medium flex items-center gap-1.5 mt-1 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-100">
                        ...
                        <CircularStatCard
                          label="TAGIHAN"
                          icon={Banknote}
                          percentage={terminPct ? parseFloat(terminPct) : 0}
                          dropShadowColor="blue"
                          subContent={
                            <div className="flex flex-col items-center mt-1">
                              <span className="text-[12px] md:text-sm font-bold uppercase tracking-wider text-slate-800">
                                Termin {toRoman(String(terminNum))}
                              </span>
                              <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wider mt-1 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-100">
                        ...
                        <StatCard
                          icon={Clock}
                          label="Sisa Waktu"
                          value={sisaWaktuInfo.value}
                          sub={sisaWaktuInfo.sub}
                          status={sisaWaktuInfo.status}
                        />
                      </div>
                      ...
                      <div className="lg:col-span-4 flex flex-col h-[380px] lg:h-full min-h-0">
                        <div className="bg-white rounded-[24px] p-4 md:p-5 border border-slate-100 shadow-sm flex flex-col h-full overflow-hidden">
                          <h3 className="text-[10px] font-bold text-slate-500 mb-2 tracking-wider uppercase flex justify-between items-center shrink-0">
                            Log Aktivitas{' '}
                            <div className="w-1.5 h-1.5 rounded-full bg-rose-400 animate-pulse" />
                          </h3>
                          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col divide-y divide-slate-100 min-h-0">
                            {persiapanSidebarFeeds.length > 0 ? (
                              ...
                              <div className="text-center text-slate-400 text-xs font-bold py-10 opacity-50 m-auto">
                                Belum ada aktivitas
                              </div>
                            )}
                          </div>
                    ...
                    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:h-[550px]">
                      ...
                      <div className="lg:col-span-7 bg-white p-6 md:p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col h-[450px] lg:h-full min-h-0 overflow-hidden">
                        <div className="flex justify-between items-center mb-6 shrink-0">
                          ...
                          <button
                            onClick={() => setShowEditProjectModal(true)}
                            className="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase flex items-center gap-2"
                          >
                            <TrendingUp size={12} /> Input Kurva
                          </button>
                        </div>
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                                    ...
                                    <div className="flex gap-6 mb-2">
                                      {(p.payload || []).map((entry, index) => (
                                        ...
                                        >
                                          ...
                                          ></span>
                                          <span className="text-[10px] uppercase font-bold text-slate-900">
                                            {safeRender(entry.value)}
                                          </span>
                                        </div>
                                      ))}
                                    </div>
                              ...
                              <Area
                                ...
                                stroke="#06b6d4"
                                strokeWidth={2}
                                fill="url(#colorRencanaDash)"
                                name="Rencana (M=Mingguan)"
                              ...
                              <Area
                                ...
                                stroke="#3b82f6"
                                strokeWidth={3}
                                fill="url(#colorRealisasiDash)"
                                name="Progres Fisik (M=Mingguan)"
                                dot={{
                                  r: 3.5,
                                  strokeWidth: 2,
                                  fill: '#ffffff',
                      ...
                      <div className="lg:col-span-5 bg-white/80 backdrop-blur-xl p-5 md:p-6 lg:p-8 rounded-[32px] border border-slate-200/60 shadow-lg text-left flex flex-col relative group overflow-hidden h-[450px] lg:h-full">
                        ...
                        <div className="flex justify-between items-center mb-5 border-b border-slate-200/60 pb-4 shrink-0 relative z-10">
                          <div className="flex items-center gap-3">
                            <div className="p-2 bg-blue-50/80 text-blue-600 rounded-xl shadow-sm border border-blue-100/50">
                              <Ruler size={16} strokeWidth={2.5} />
                            </div>
                            <h3 className="text-sm font-black text-slate-800 tracking-tight uppercase">
                              Progress Item Pekerjaan
                            </h3>
                          </div>
                        </div>
                          ...
                          projectData.item_utama_data.length === 0 ? (
                            <div className="text-center py-10 bg-slate-50/30 rounded-2xl border border-dashed border-slate-200 m-auto w-full">
                              ...
                              <p className="text-xs font-bold text-slate-400">
                                Belum ada item utama yang diatur.
                              </p>
                            </div>
                          ) : (
                              ...
                              <table className="w-full text-left border-collapse">
                                <thead className="sticky top-0 bg-white/40 backdrop-blur-md z-20">
                                  <tr className="border-b border-slate-200/60">
                                    <th className="pb-3 pr-2 text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">
                                      Item Pekerjaan
                                    </th>
                                    <th className="pb-3 px-1 text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] text-center">
                                      Renc. Hari Ini
                                    </th>
                                    <th className="pb-3 px-1 text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] text-center">
                                      Total
                                    ...
                                    <th className="pb-3 pl-3 text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] text-right">
                                      Progress
                                    </th>
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100/50">
                                  {projectData.item_utama_data.map(
                                    (item, idx) => (
                                      <tr
                                        key={item.id || idx}
                                        className="hover:bg-blue-50/40 transition-colors group border-b border-slate-50 last:border-0"
                                      >
                                        <td className="py-4 pr-2 max-w-[120px]">
                                          <span
                                            className="text-[11px] font-black text-slate-800 leading-tight block truncate uppercase"
                                            title={item.nama}
                                          >
                                            {item.nama}
                                          </span>
                                        ...
                                        <td className="py-4 pl-3 text-right">
                                          <span className="text-[13px] font-black text-blue-600 drop-shadow-sm">
                                            {item.persen}%
                                          </span>
                                        </td>
                                      </tr>
                    ...
                    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:h-[360px]">
                      ...
                      <div
                        className={`lg:col-span-5 bg-white p-5 md:p-6 rounded-[32px] border flex flex-col h-[350px] lg:h-full relative transition-all duration-300 min-h-0 overflow-hidden ${
                          isDraggingReport
                            ? 'border-rose-400 shadow-xl scale-[1.02] bg-rose-50/30'
                      ...
                      >
                        {isDraggingReport && (
                          <div className="absolute inset-0 z-50 bg-white/90 backdrop-blur-sm rounded-[32px] flex flex-col items-center justify-center border-2 border-dashed border-rose-400 m-2 pointer-events-none">
                            <div className="bg-rose-100 p-4 rounded-full shadow-sm text-rose-600 mb-3 animate-bounce">
                              <FileText size={32} />
                            </div>
                        ...
                        <div className="flex justify-between items-center mb-5 shrink-0 border-b border-slate-100 pb-4">
                          <h3 className="text-[11px] font-black text-slate-800 uppercase flex items-center gap-2 tracking-wider">
                            <FileText size={16} className="text-rose-500" />{' '}
                            Laporan
                          </h3>
                          <button
                            onClick={() => reportFileInputRef.current?.click()}
                            className="bg-rose-50 text-rose-600 px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase flex items-center gap-1.5 hover:bg-rose-100 transition-colors shadow-sm"
                          >
                            {isProcessing ? (
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 flex flex-col min-h-0">
                          {weeklyReports.length > 0 ? (
                            <div className="flex-1 flex flex-col gap-4 pb-2">
                              {weeklyReports.slice(0, 1).map((doc) => (
                                <div
                                  ...
                                  className="group border border-slate-200 rounded-3xl overflow-hidden hover:shadow-2xl hover:border-blue-400 hover:-translate-y-1.5 transition-all duration-300 cursor-pointer bg-white relative flex flex-col flex-1 min-h-[220px]"
                                >
                                  ...
                                  <button
                                    ...
                                    className="absolute top-3 right-3 p-2 bg-white/90 text-rose-500 rounded-xl opacity-0 group-hover:opacity-100 transition-all z-20 hover:bg-rose-50 shadow-md backdrop-blur-sm"
                                  >
                                    <Trash size={14} />
                                  </button>
                                  <div className="absolute inset-x-0 bottom-0 pt-24 pb-4 px-5 bg-gradient-to-t from-slate-900/95 via-slate-900/60 to-transparent flex flex-col justify-end z-10 pointer-events-none">
                                    <div className="flex justify-between items-end gap-3">
                                      <div className="flex flex-col">
                                        <h4
                                          className="text-[11px] font-bold text-white line-clamp-2 leading-snug mb-1 break-all group-hover:text-blue-300 transition-colors"
                                          title={doc.name}
                                        >
                                          {String(doc.name || '')}
                                        </h4>
                                      ...
                                      <div className="bg-rose-500/90 backdrop-blur-sm border border-rose-400 text-white text-[8px] font-black tracking-widest px-2 py-1.5 rounded-lg shadow-sm shrink-0 uppercase">
                                        PDF
                                      </div>
                                    </div>
                                  </div>
                            ...
                            >
                              ...
                              <p className="text-xs font-black text-slate-700 mb-1 uppercase tracking-widest">
                                Upload Laporan
                              </p>
                              <p className="text-[9px] font-bold uppercase tracking-widest leading-relaxed text-slate-400">
                                Tarik & Lepas File PDF
                                <br />
                                atau Klik di Sini
                              </p>
                            </div>
                      ...
                      <div className="lg:col-span-7 bg-white p-5 md:p-6 lg:p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col text-left h-[350px] lg:h-full relative group overflow-hidden">
                        ...
                        <div className="flex justify-between items-center w-full mb-5 border-b border-slate-100 pb-4 relative z-10 shrink-0">
                          <span className="text-slate-800 text-[11px] uppercase font-black tracking-wider leading-tight flex items-center gap-2">
                            <ImageIcon size={16} className="text-blue-500" />{' '}
                            Dokumentasi Lapangan
                          </span>
                          <button
                            onClick={() => setActiveMenu('dokumentasi')}
                            className="px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-transform duration-500 shadow-sm bg-blue-50 text-blue-600 border border-blue-100/80 hover:bg-blue-100 cursor-pointer pointer-events-auto"
                            title="Lihat Galeri"
                          >
                            Lihat Semua
                          </button>
                          ...
                          {photos.length > 0 ? (
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                              {photos.slice(0, 3).map((item, idx) => (
                                <div
                                  key={item.display_id || item.id || idx}
                                  className="w-full flex flex-col gap-2 group/item cursor-pointer"
                                ...
                                >
                                  ...
                                  <div>
                                    <h4 className="text-[10px] font-bold text-slate-800 line-clamp-1 group-hover/item:text-blue-600 transition-colors">
                                      {String(item.title || '')}
                                    </h4>
        ...
        {activeMenu === 'map' && (
          <div className="h-full p-4 md:p-8">
            <SiteMapView
              projectData={projectData}
              onUpdateRoutes={handleRoutesUpdate}
        ...
        {activeMenu === 'contract' && (
          <div className="h-full flex flex-col overflow-hidden">
            <div className="px-6 py-6 md:px-10 flex justify-between items-center border-b border-slate-200 bg-white/50 backdrop-blur-md">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-white shadow-sm border border-slate-100 rounded-2xl hidden sm:block">
                  <Briefcase className="text-blue-500" />
                </div>
                <div>
                  <h3 className="text-xl font-bold">
                    DATA KONTRAK (DATA INFORMASI)
                  </h3>
              ...
              {!isEditingContract ? (
                <button
                  ...
                  className="bg-white px-6 py-3 rounded-2xl text-xs font-bold uppercase shadow-sm border border-slate-200 flex gap-2"
                >
                  <Edit3 size={16} className="text-blue-500" /> EDIT DATA
                </button>
              ) : (
                <div className="flex gap-3">
                  <button
                    onClick={() => setIsEditingContract(false)}
                    className="bg-white px-6 py-3 rounded-2xl text-xs font-bold uppercase border border-slate-200"
                  >
                    BATAL
                  </button>
                  <button
                    onClick={handleUpdateStakeholder}
                    className="bg-blue-600 text-white px-6 py-3 rounded-2xl text-xs font-bold uppercase shadow-md flex gap-2"
                  >
                    <Save size={16} /> SIMPAN
                  </button>
                </div>
            ...
            <div className="p-6 md:p-10 flex-1 overflow-y-auto custom-scrollbar">
              {!isEditingContract ? (
                <div className="flex flex-col gap-12 w-full mx-auto">
                  ...
                  />
                  <ContractTable
                    title="DATA INFORMASI KONTRAKTOR / PELAKSANA"
                    icon={<HardHat size={16} />}
                    dataArray={kontraktorData}
                    colorClass="text-amber-600"
                    bgClass="bg-amber-100"
                  />
                  <ContractTable
                    title="DATA INFORMASI KONSULTAN SUPERVISI/PENGAWAS"
                    icon={<ShieldCheck size={16} />}
                    dataArray={konsultanData}
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 md:p-10 w-full mx-auto space-y-12">
                  <div className="space-y-4">
                    <div className="mb-2 border-b border-slate-200 pb-4">
                      <h3 className="text-base font-black uppercase text-blue-600">
                        1. INPUT DATA OWNER (DINAS)
                      </h3>
                    </div>
                      ...
                      <div className="hidden sm:flex bg-slate-200 border-b border-slate-300 py-3.5 px-6 justify-between items-center">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="text-[10px] font-bold text-slate-600 bg-white border border-slate-200 shadow-sm px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-slate-50"
                          >
                            <Plus size={12} /> Keterangan
                          </button>
                          <button
                            type="button"
                            onClick={() =>
                            ...
                            className="text-[10px] font-bold text-blue-600 bg-blue-50 border border-blue-200 shadow-sm px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-blue-100"
                          >
                            <Plus size={12} /> Personil
                          </button>
                        </div>
                      </div>
                      <div className="p-4 sm:p-6 space-y-3">
                        {(masterForm.dinas || []).map((d, i) => {
                          ...
                          return (
                            <div
                              key={i}
                              className="flex flex-col md:flex-row gap-4 p-4 bg-slate-50 border border-slate-100 rounded-2xl group items-start"
                            >
                              <div className="w-full md:w-1/3">
                                {isPersonil ? (
                                  <input
                                    ...
                                    className="w-full p-2.5 rounded-xl border border-slate-200 text-sm font-bold bg-white focus:outline-blue-400"
                                    value={d.name}
                                    onChange={(e) => {
                                      const newD = [...masterForm.dinas];
                                  ...
                                  <input
                                    type="text"
                                    placeholder="Label (Misal: Instansi)"
                                    className="w-full p-2.5 rounded-xl border border-slate-200 text-sm font-bold bg-white focus:outline-blue-400"
                                    value={d.role}
                                    onChange={(e) => {
                                      const newD = [...masterForm.dinas];
                              ...
                              <div className="flex-1 w-full flex flex-col gap-2">
                                {isPersonil ? (
                                  <>
                                    <input
                                      type="text"
                                      placeholder="Jabatan (Tampil di Kiri Tabel)"
                                      className="w-full p-2.5 rounded-xl border border-slate-200 text-sm font-bold bg-white focus:outline-blue-400"
                                      value={d.role}
                                      onChange={(e) => {
                                        const newD = [...masterForm.dinas];
                                  ...
                                  <>
                                    <input
                                      type="text"
                                      placeholder="Nama Instansi/Perangkat Daerah"
                                      className="w-full p-2.5 rounded-xl border border-slate-200 text-sm font-bold bg-white focus:outline-blue-400"
                                      value={d.name}
                                      onChange={(e) => {
                                        const newD = [...masterForm.dinas];
                              ...
                              {masterForm.dinas.length > 1 && (
                                <button
                                  ...
                                  className="p-2.5 text-rose-400 bg-rose-50 rounded-xl hover:bg-rose-100 transition-colors shrink-0"
                                >
                                  <Trash size={16} />
                                </button>
                              )}
                  ...
                  <div className="space-y-4">
                    <div className="mb-2 border-b border-slate-200 pb-4">
                      <h3 className="text-base font-black uppercase text-amber-600">
                        2. INPUT DATA KONTRAKTOR / PELAKSANA
                      </h3>
                    </div>
                    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                      <div className="hidden sm:flex bg-slate-200 border-b border-slate-300 py-3.5 px-6">
                        <div className="w-1/3 text-xs font-black uppercase tracking-wider text-slate-800">
                          Informasi / Entitas
                        </div>
                        <div className="w-2/3 text-xs font-black uppercase tracking-wider text-slate-800 pl-4 border-l border-slate-300/60">
                      ...
                      <div className="flex flex-col">
                        {masterForm.kontraktor.fields.map((f, i) => (
                          <div
                            key={f.id}
                            className="flex flex-col sm:flex-row py-3 px-6 border-b border-slate-100 last:border-0 bg-white hover:bg-slate-50/50 transition-colors gap-3 sm:gap-4 items-start sm:items-center"
                          >
                            <div className="w-full sm:w-1/3 flex gap-2">
                              <select
                                className="p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-[10px] font-bold outline-none focus:border-blue-400 shrink-0"
                                value={f.type}
                                onChange={(e) => {
                                  const newF = [
                                    ...masterForm.kontraktor.fields,
                                  ];
                              ...
                              <input
                                ...
                                className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-[11px] font-bold uppercase text-slate-600 outline-none focus:border-blue-400"
                                value={f.label}
                                onChange={(e) => {
                                  const newF = [
                                    ...masterForm.kontraktor.fields,
                            ...
                            <div className="w-full sm:w-2/3 flex gap-2 items-center">
                              {f.type === 'date' ? (
                                <input
                                  type="date"
                                  className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-bold text-slate-800 focus:bg-white focus:border-blue-400 focus:outline-none transition-all"
                                  value={f.value}
                                  onChange={(e) => {
                                    const newF = [
                                      ...masterForm.kontraktor.fields,
                                ...
                                <AutoResizeTextarea
                                  ...
                                  placeholder="Isi detail keterangan..."
                                  className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-bold text-slate-800 focus:bg-white focus:border-blue-400 focus:outline-none transition-all leading-relaxed"
                                />
                              )}
                              <div className="flex gap-1 shrink-0">
                                <button
                                  ...
                                  disabled={i === 0}
                                  className="p-2 text-slate-400 bg-slate-100 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors disabled:opacity-30"
                                  title="Geser ke Atas"
                                >
                                  <ArrowUp size={14} />
                                </button>
                                <button
                                  ...
                                  disabled={
                                    i ===
                                    masterForm.kontraktor.fields.length - 1
                                  }
                                  className="p-2 text-slate-400 bg-slate-100 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors disabled:opacity-30"
                                  title="Geser ke Bawah"
                                >
                                ...
                                <button
                                  ...
                                  className="p-2 text-rose-400 bg-rose-50 rounded-lg hover:bg-rose-100 transition-colors ml-1"
                                  title="Hapus Field"
                                >
                                  <Trash size={14} />
                                </button>
                      ...
                      <div className="p-4 sm:px-6 bg-[#f8fafc] border-t border-slate-200 flex justify-end">
                        <button
                          ...
                          className="text-[10px] font-bold text-blue-600 bg-white border border-slate-200 shadow-sm px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-slate-50 w-max"
                        >
                            ...
                            onClick={() =>
                              setMasterForm({
                                ...
                                kontraktor: {
                                  ...masterForm.kontraktor,
                                  personil: [
                                    ...(masterForm.kontraktor.personil || []),
                                    { name: '', position: '' },
                                  ],
                                },
                              })
                            }
                            className="text-[10px] font-bold bg-white border border-slate-200 shadow-sm px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-slate-50"
                          >
                            <Plus size={12} />
                            Tambah Personil
                          </button>
                        ...
                        {(masterForm.kontraktor.personil || []).map((p, i) => (
                          ...
                          >
                            <input
                              type="text"
                              placeholder="Nama Lengkap"
                              className="flex-1 w-full p-2.5 rounded-xl border border-slate-200 bg-white text-sm focus:outline-blue-400"
                              value={p.name}
                              onChange={(e) => {
                                const newP = [
                                  ...masterForm.kontraktor.personil,
                            ...
                            <input
                              type="text"
                              placeholder="Jabatan"
                              className="w-full md:w-1/3 p-2.5 rounded-xl border border-slate-200 bg-white text-sm focus:outline-blue-400"
                              value={p.position}
                              onChange={(e) => {
                                const newP = [
                                  ...masterForm.kontraktor.personil,
                                ];
                                newP[i].position = e.target.value;
                                setMasterForm({
                                  ...masterForm,
                                  kontraktor: {
                                    ...masterForm.kontraktor,
                            ...
                            <button
                              ...
                              className="p-2.5 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-100 transition-colors w-full md:w-auto flex justify-center"
                            >
                              <Trash size={16} />
                            </button>
                          </div>
                  ...
                  <div className="space-y-4">
                    <div className="mb-2 border-b border-slate-200 pb-4">
                      <h3 className="text-base font-black uppercase text-emerald-600">
                        3. INPUT DATA KONSULTAN SUPERVISI
                      </h3>
                    </div>
                    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                      <div className="hidden sm:flex bg-slate-200 border-b border-slate-300 py-3.5 px-6">
                            ...
                            <div className="w-full sm:w-2/3 flex gap-2 items-center">
                              ...
                              <div className="flex gap-1 shrink-0">
                                ...
                                <button
                                  ...
                                  disabled={
                                    i === masterForm.konsultan.fields.length - 1
                                  }
                                  className="p-2 text-slate-400 bg-slate-100 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors disabled:opacity-30"
                                  title="Geser ke Bawah"
                                >
                        ...
                        {(masterForm.konsultan.personil || []).map((p, i) => (
                          ...
                          >
                            <input
                              ...
                              value={p.name}
                              onChange={(e) => {
                                const newP = [...masterForm.konsultan.personil];
                                newP[i].name = e.target.value;
                                setMasterForm({
                                  ...masterForm,
                                  konsultan: {
          ...
          <div className="h-full flex flex-col">
            <header className="px-6 py-6 md:px-10 flex justify-between items-center border-b border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold">Arsip Dokumen Proyek</h2>
                </div>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowCategoryManager(true)}
                  className="bg-white border px-4 py-3 rounded-2xl flex items-center gap-2"
                >
                  <Settings size={16} /> Kategori
                </button>
                <button
                  onClick={() => setShowDocModal(true)}
                  className="bg-blue-600 text-white px-4 py-3 rounded-2xl flex items-center gap-2"
                >
            ...
            <div className="flex-1 p-4 md:p-8 overflow-y-auto custom-scrollbar">
              <div className="mb-6 space-y-4">
                <div className="bg-white p-2 flex items-center rounded-2xl border border-slate-200 shadow-sm focus-within:border-blue-400 focus-within:shadow-md transition-all">
                  ...
                  <input
                    type="text"
                    placeholder="Cari nama dokumen atau arsip..."
                    value={docSearchQuery}
                    onChange={(e) => setDocSearchQuery(e.target.value)}
                    className="w-full py-3 px-2 bg-transparent outline-none text-sm font-bold text-slate-700"
                  />
                  {docSearchQuery && (
                    <button
                      onClick={() => setDocSearchQuery('')}
                      className="pr-4 pl-2 text-rose-400 hover:text-rose-600 transition-colors"
                    >
                      <X size={20} />
                    </button>
                  )}
                ...
                <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2 pt-1">
                  <button
                    onClick={() => setDocFilterCategory('Semua')}
                ...
                {filteredDocs.length > 0 ? (
                  filteredDocs.map((doc) => {
                    ...
                    return (
                      <div
                        key={doc.id}
                        className="bg-white border p-4 rounded-2xl flex items-center justify-between group hover:border-blue-300 hover:shadow-md transition-all"
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`p-3 rounded-xl ${info.bg} ${info.color}`}
                          >
                            <info.icon size={20} />
                          </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            ...
                            className="p-2 text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity bg-blue-50 rounded-lg hover:bg-blue-100"
                          ...
                          <button
                            onClick={() =>
                              setDeleteConfig({ id: doc.id, type: 'doc' })
                            }
                            className="p-2 text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity bg-rose-50 rounded-lg hover:bg-rose-100"
                            title="Hapus Dokumen"
                          >
                            <Trash size={16} />
                          </button>
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="bg-slate-50 border border-slate-200 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 p-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-all"
                          >
                  ...
                  <div className="flex flex-col items-center justify-center py-16 text-center bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
                    ...
                    <p className="text-xs text-slate-500 font-medium mt-2">
                      Tidak ada dokumen yang sesuai dengan filter atau kata
                    ...
                    {(docSearchQuery || docFilterCategory !== 'Semua') && (
                      <button
                        onClick={() => {
                          setDocSearchQuery('');
                          setDocFilterCategory('Semua');
                        }}
                        className="mt-4 text-[10px] font-bold bg-white border border-slate-200 text-blue-600 px-4 py-2 rounded-xl shadow-sm hover:bg-blue-50 transition-colors"
                      >
            ...
            <form onSubmit={handleUpdateProject} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  value={editProjectForm.pekerjaan}
                  onChange={(e) =>
                    setEditProjectForm({
                      ...editProjectForm,
                      pekerjaan: e.target.value,
              ...
              <div>
                ...
                <input
                  type="text"
                  className="w-full p-3 rounded-xl border bg-slate-50 outline-none focus:border-blue-400"
                  value={editProjectForm.termin_ke}
                  onChange={(e) =>
                    setEditProjectForm({
                      ...editProjectForm,
                      termin_ke: e.target.value,
                    })
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  step="0.01"
                  className="w-full p-3 rounded-xl border bg-slate-50 outline-none focus:border-blue-400"
                  value={editProjectForm.termin_persen}
                  onChange={(e) =>
                    setEditProjectForm({
                      ...editProjectForm,
                      termin_persen: e.target.value,
                    })
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="number"
                  className="w-full p-3 rounded-xl border bg-slate-50 outline-none focus:border-blue-400"
                  value={editProjectForm.waktu_pelaksanaan}
                  onChange={(e) =>
                    setEditProjectForm({
                      ...editProjectForm,
              ...
              <div>
                ...
                <input
                  type="text"
                  className="w-full p-3 rounded-xl border bg-slate-50 outline-none focus:border-blue-400"
                  value={editProjectForm.panjang_rencana}
                  onChange={(e) =>
                    setEditProjectForm({
                      ...editProjectForm,
                      panjang_rencana: e.target.value,
                    })
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  className="w-full p-3 rounded-xl border bg-slate-50 outline-none focus:border-blue-400"
                  value={editProjectForm.lebar_rencana}
                  onChange={(e) =>
                    setEditProjectForm({
                      ...editProjectForm,
                      lebar_rencana: e.target.value,
                    })
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  className="w-full p-3 rounded-xl border bg-slate-50 outline-none focus:border-blue-400"
                  value={editProjectForm.jenis_model}
                  onChange={(e) =>
                    setEditProjectForm({
                      ...editProjectForm,
                      jenis_model: e.target.value,
                    })
                  }
                  placeholder="Misal: U-Ditch, Batu Kali..."
                />
              </div>

              <div className="border-t border-slate-200 pt-5 mt-4">
                <div className="flex justify-between items-center mb-3">
                  <label className="text-[11px] font-black uppercase text-blue-600 tracking-widest">
                    Item Utama Proyek
                  </label>
                  <button
                    type="button"
                    ...
                    className="text-[9px] font-bold bg-blue-50 border border-blue-200 text-blue-600 px-3 py-1.5 rounded-lg flex items-center gap-1 shadow-sm"
                  >
                ...
                <div className="space-y-3">
                  {(editProjectForm.item_utama_data || []).map((item, idx) => (
                    <div
                      key={item.id || idx}
                      className="flex flex-col gap-2 bg-slate-50 p-3 rounded-xl border border-slate-200 shadow-sm w-full"
                    >
                      <div className="flex gap-2 items-center">
                        <input
                          type="text"
                          placeholder="Misal: Pekerjaan Galian"
                          className="w-full p-2 rounded-lg border border-slate-200 text-xs font-bold outline-none focus:border-blue-400"
                          value={item.nama}
                          onChange={(e) => {
                            const n = [...editProjectForm.item_utama_data];
                            n[idx].nama = e.target.value;
                            setEditProjectForm({
                              ...editProjectForm,
                              item_utama_data: n,
                            });
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        <input
                          type="text"
                          placeholder="Renc. Hari Ini"
                          className="w-full p-2 rounded-lg border border-slate-200 text-xs outline-none focus:border-blue-400 text-center"
                          value={item.bobot || ''}
                          onChange={(e) => {
                            const n = [...editProjectForm.item_utama_data];
                            ...
                            n[idx].persen = parseFloat(
                              ((valT * valH) / 100).toFixed(2)
                            );
                            setEditProjectForm({
                              ...editProjectForm,
                              item_utama_data: n,
                        ...
                        <input
                          type="number"
                          placeholder="Progres %"
                          className="w-full p-2 rounded-lg border border-transparent bg-slate-100 text-blue-600 text-xs outline-none font-black text-center cursor-not-allowed shadow-inner"
                          value={item.persen}
                          readOnly
                          title="Progress Otomatis: (Total x Hari Ini) / 100"
                        />
                      </div>
                    </div>
                  ))}
                  {(!editProjectForm.item_utama_data ||
                    editProjectForm.item_utama_data.length === 0) && (
                    <p className="text-[10px] text-slate-400 text-center italic py-2 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                      Klik 'Tambah Item' untuk memasukkan data progres item
                      utama.
                    </p>
                  )}
                </div>
              ...
              <div className="border-t border-slate-200 pt-5 mt-4">
                <div className="mb-3">
                  <label className="text-[11px] font-black uppercase text-blue-600 tracking-widest">
                    Data Kurva S Pekerjaan
                  </label>
                </div>
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-bold block mb-1">
                      Rencana (%) - pisah dengan strip (-)
                    </label>
                    <textarea
                      className="w-full p-4 rounded-xl border font-mono bg-slate-50 outline-none focus:border-blue-400 text-sm"
                      rows="3"
                      value={sCurveForm.plan}
                      onChange={(e) =>
                        setSCurveForm({ ...sCurveForm, plan: e.target.value })
                      }
                      placeholder="Contoh: 0 - 5.5 - 12 - 25"
                    ></textarea>
                    {/* Live Preview Label M1, M2 */}
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {String(sCurveForm.plan || '')
                        .split('-')
                        .map((val, idx) =>
                          val.trim() !== '' ? (
                            <div
                              key={idx}
                              className="text-[9px] font-bold px-2 py-1 rounded-md border bg-blue-50 text-blue-600 border-blue-200"
                            >
                              M{idx + 1}{' '}
                  ...
                  <div>
                    <label className="text-[10px] font-bold block mb-1">
                      Realisasi (%) - pisah dengan strip (-)
                    </label>
                    <textarea
                      className="w-full p-4 rounded-xl border font-mono bg-slate-50 outline-none focus:border-blue-400 text-sm"
                      rows="3"
                      value={sCurveForm.actual}
                      onChange={(e) =>
                        setSCurveForm({ ...sCurveForm, actual: e.target.value })
                      }
                      placeholder="Contoh: 0 - 6 - 15"
                    ></textarea>
                    {/* Live Preview Label M1, M2 */}
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {String(sCurveForm.actual || '')
                        .split('-')
                        .map((val, idx) =>
                          val.trim() !== '' ? (
                            <div
                              key={idx}
                              className="text-[9px] font-bold px-2 py-1 rounded-md border bg-emerald-50 text-emerald-600 border-emerald-200"
                            >
                              M{idx + 1}{' '}
                              <span className="opacity-40 mx-0.5">|</span>{' '}
                              {val.trim()}%
              ...
              <button
                type="submit"
                disabled={isProcessing}
                className="w-full bg-blue-600 text-white py-4 rounded-2xl text-xs font-bold uppercase mt-4 shadow-md"
              >
                {isProcessing ? (
                  <Loader2 size={16} className="animate-spin mx-auto" />
                ) : (
          ...
          <div className="bg-white rounded-[32px] p-6 md:p-10 w-full max-w-5xl shadow-2xl relative my-auto">
            ...
            <h3 className="text-2xl font-black mb-6 pr-10 border-b border-slate-200 pb-4 text-slate-800">
              {reportTab === 'harian'
                ...
                : 'Input Lapor Lapangan'}
            </h3>

            {/* TAB SWITCHER */}
            <div className="flex gap-2 bg-slate-100 p-1.5 rounded-xl border text-center mb-6">
              <button
                onClick={() => setReportTab('harian')}
                className={`flex-1 py-3 rounded-lg text-[11px] md:text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${
                  reportTab === 'harian'
                    ? 'bg-blue-600 text-white shadow-md'
                    : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50'
                }`}
              ...
              >
                ...
                <div className="bg-slate-50 p-6 md:p-8 rounded-2xl border border-slate-100 space-y-5 w-full">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200/50 pb-2 gap-2">
                    <h4 className="text-sm font-black uppercase text-slate-500 tracking-widest">
                      A. Jam Kerja & Lokasi
                    </h4>
                    <span className="text-[9px] font-bold text-slate-400 bg-white px-2 py-1 rounded border border-slate-200 flex items-center gap-1.5">
                      <Save size={10} className="text-blue-500" /> Waktu Kerja &
                      Lokasi otomatis tersimpan ke template rutinitas
                    </span>
                  </div>
                  ...
                  <SurveyInputRow label="Tanggal Laporan & Lokasi">
                    <div className="flex flex-col sm:flex-row gap-3">
                      <input
                        ...
                        className="w-full sm:w-1/3 p-3.5 rounded-xl border border-slate-200 bg-white text-sm font-bold text-slate-800 outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-50 transition-all shadow-sm"
                        required
                      />
                      <input
                        type="text"
                        value={dailyReportForm.lokasi}
                        onChange={(e) =>
                          setDailyReportForm((p) => ({
                            ...p,
                        ...
                        placeholder="Nama Jalan, Titik Lokasi, atau STA (Opsional)"
                        className="w-full p-3.5 rounded-xl border border-slate-200 bg-white text-sm font-bold text-slate-800 outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-50 transition-all shadow-sm"
                      />
                    </div>
                  </SurveyInputRow>
                  ...
                  {dailyReportForm.shifts.map((shift, idx) => (
                    <div
                      key={shift.id}
                      className="relative bg-white p-5 rounded-xl border border-slate-200 shadow-sm"
                    >
                      {dailyReportForm.shifts.length > 1 && (
                        <button
                          ...
                          className="absolute top-4 right-4 text-rose-400 hover:text-rose-600 bg-rose-50 hover:bg-rose-100 p-1.5 rounded-lg transition-colors"
                          title="Hapus Shift"
                        >
                          <Trash size={14} />
                        </button>
                        ...
                        <SurveyInputRow label="Waktu Mulai">
                          <div className="flex gap-2">
                            <input
                              ...
                              className="w-full p-3 rounded-lg border border-slate-200 bg-slate-50 outline-none focus:bg-white focus:border-blue-400 text-xs font-bold transition-colors"
                            />
                            <input
                              type="time"
                              value={shift.jamMulai}
                              onChange={(e) =>
                                handleShiftChange(
                                  idx,
                                  'jamMulai',
                              ...
                              className="w-28 p-3 rounded-lg border border-slate-200 bg-slate-50 outline-none focus:bg-white focus:border-blue-400 text-xs font-bold shrink-0 transition-colors"
                            />
                          </div>
                        </SurveyInputRow>
                        ...
                        <SurveyInputRow label="Waktu Selesai (Auto)">
                          <div className="flex gap-2">
                            <input
                              ...
                              className="w-full p-3 rounded-lg border border-slate-200 bg-slate-50 outline-none focus:bg-white focus:border-blue-400 text-xs font-bold transition-colors"
                              title="Otomatis menyesuaikan jika lintas jam malam"
                            />
                            <input
                              type="time"
                              value={shift.jamSelesai}
                              onChange={(e) =>
                                handleShiftChange(
                                  idx,
                                  'jamSelesai',
                ...
                <div className="bg-slate-50 p-6 md:p-8 rounded-2xl border border-slate-100 space-y-3 w-full">
                  ...
                  <div className="flex flex-col bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm w-full">
                    <div className="hidden md:flex bg-[#f8fafc] border-b border-slate-200 px-4 py-3 items-center text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                      <div className="w-1/2 md:w-2/3 text-left">
                        Waktu Rentang Kerja
                      </div>
                    ...
                    {Object.keys(dailyReportForm.cuaca).map((jam) => (
                      <div
                        key={jam}
                        className="flex border-b border-slate-100 last:border-b-0 items-center hover:bg-slate-50/50 transition-colors w-full"
                      >
                        <label className="w-1/2 md:w-2/3 p-4 bg-slate-50/80 border-r border-slate-100 text-xs font-bold text-slate-600">
                          {jam}
                        </label>
                        <div className="flex-1 p-2 px-4 shrink-0">
                          <select
                            ...
                            onChange={(e) =>
                              setDailyReportForm((p) => ({
                                ...p,
                                cuaca: { ...p.cuaca, [jam]: e.target.value },
                              }))
                            }
                ...
                <div className="bg-slate-50 p-6 md:p-8 rounded-2xl border border-slate-100 space-y-3 w-full">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200/50 pb-2 gap-2">
                    ...
                    <span className="text-[9px] font-bold text-slate-400 bg-white px-2 py-1 rounded border border-slate-200 flex items-center gap-1.5">
                      <Save size={10} className="text-blue-500" /> Item otomatis
                      tersimpan sebagai template rutinitas
                    </span>
                  </div>
                  <div className="flex flex-col bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm w-full">
                    <div className="hidden md:flex bg-[#f8fafc] border-b border-slate-200 pl-4 pr-12 py-3 items-center text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                      ...
                      <div className="w-[18%]">Kemaren</div>
                      <div className="w-[18%]">Hari Ini</div>
                      <div className="w-[18%]">Total</div>
                      <div className="w-[18%]">Satuan</div>
                    </div>
                    {dailyReportForm.aktivitas.map((akt, i) => {
                      const total =
                        (parseFloat(akt.kemarin) || 0) +
                        (parseFloat(akt.hariIni) || 0);
                      return (
                        <div
                          key={i}
                          className="flex flex-col md:flex-row items-center border-b border-slate-100 last:border-b-0 hover:bg-slate-50/50 transition-colors w-full p-2 pr-12 md:py-0 md:pl-0 md:pr-12 relative group/akt"
                        >
                          <div className="w-full md:flex-1 min-w-[200px] p-2 md:px-4">
                            <input
                              type="text"
                              className="w-full p-2 text-xs font-bold text-slate-700 border-none bg-transparent outline-none focus:bg-slate-50 rounded"
                              value={akt.nama}
                              onChange={(e) => {
                              ...
                              placeholder="Ketik Nama Item Pekerjaan..."
                            />
                          </div>
                          <div className="flex w-full md:w-[72%] gap-2 md:gap-0 mt-2 md:mt-0 divide-x divide-slate-100">
                            <div className="flex-1 p-1 md:p-2">
                              <input
                                type="number"
                                placeholder="0"
                                className="w-full p-2 rounded-lg border bg-white text-xs font-black text-center outline-none focus:border-blue-400"
                                value={akt.kemarin}
                                onChange={(e) => {
                                  const n = [...dailyReportForm.aktivitas];
                                  n[i].kemarin = e.target.value;
                                  setDailyReportForm({
                                    ...dailyReportForm,
                                    aktivitas: n,
                                  });
                            ...
                            <div className="flex-1 p-1 md:p-2">
                              <input
                                type="number"
                                className="w-full p-2 rounded-lg border border-transparent bg-slate-50 text-blue-600 font-black text-xs text-center cursor-not-allowed"
                                value={total > 0 ? total : ''}
                                readOnly
                              />
                            </div>
                            <div className="flex-1 p-1 md:p-2">
                              <input
                                type="text"
                                className="w-full p-2 rounded-lg border bg-white text-xs font-bold text-center outline-none focus:border-blue-400"
                                value={akt.satuan}
                                onChange={(e) => {
                                  const n = [...dailyReportForm.aktivitas];
                    ...
                    <div className="p-4 bg-[#f8fafc] border-t border-slate-200 flex justify-end items-center">
                      <button
                        ...
                        className="text-[10px] font-bold text-blue-600 bg-white border border-slate-200 shadow-sm px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-slate-50 w-max"
                      >
                        <Plus size={12} /> Tambah Item Lainnya
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 p-6 md:p-8 rounded-2xl border border-slate-100 space-y-3 w-full">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200/50 pb-2 gap-2">
                    ...
                    <span className="text-[9px] font-bold text-slate-400 bg-white px-2 py-1 rounded border border-slate-200 flex items-center gap-1.5">
                      <Save size={10} className="text-blue-500" /> Tersimpan di
                      template rutinitas
                    </span>
                  </div>
                  <div className="flex flex-col bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm w-full">
                    ...
                    {(Array.isArray(dailyReportForm.tenagaKerja)
                      ...
                      <div
                        key={i}
                        className="flex flex-col md:flex-row border-b border-slate-100 last:border-b-0 items-center hover:bg-slate-50/50 transition-colors w-full p-2 pr-12 md:p-0 md:pr-12 relative group/tk"
                      >
                        <div className="w-full md:w-2/3 p-2 md:p-4 md:border-r border-slate-100">
                          <input
                            type="text"
                            className="w-full text-xs font-bold text-slate-700 bg-transparent outline-none focus:bg-slate-50 p-2 rounded"
                            value={tk.posisi}
                            onChange={(e) => {
                        ...
                        <div className="w-full md:flex-1 p-2 md:px-4 shrink-0">
                          <input
                            ...
                            min="0"
                            placeholder="0"
                            className="w-full p-2 text-center rounded-lg border border-slate-200 bg-white outline-none text-sm font-bold text-slate-800 focus:bg-blue-50 focus:border-blue-400 transition-all"
                            value={tk.jumlah}
                            onChange={(e) => {
                              const n = [...dailyReportForm.tenagaKerja];
                        ...
                        <button
                          ...
                          onClick={() =>
                            setDailyReportForm((p) => ({
                              ...p,
                              tenagaKerja: p.tenagaKerja.filter(
                                (_, idx) => idx !== i
                              ),
                            }))
                          }
                    ...
                    <div className="p-4 bg-[#f8fafc] border-t border-slate-200 flex justify-end items-center">
                      <button
                        ...
                        className="text-[10px] font-bold text-blue-600 bg-white border border-slate-200 shadow-sm px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-slate-50 w-max"
                      >
                        <Plus size={12} /> Tambah Posisi Lainnya
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {/* CATATAN & KENDALA */}
                <div className="bg-slate-50 p-6 md:p-8 rounded-2xl border border-slate-100 space-y-3 w-full">
                  <h4 className="text-sm font-black uppercase text-slate-500 tracking-widest border-b border-slate-200/50 pb-2 mb-2">
                    E. Catatan & Kendala Lapangan
                  </h4>
                ...
                <div className="bg-slate-50 p-6 md:p-8 rounded-2xl border border-slate-100 space-y-4 w-full">
                  <div className="flex justify-between items-center border-b border-slate-200/50 pb-2">
                    <h4 className="text-sm font-black uppercase text-slate-500 tracking-widest">
                      F. Dokumentasi Lapangan (Opsional)
                    </h4>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
                    ...
                    {repFiles &&
                      repFiles.map((file, idx) => (
                        <div
                          key={idx}
                          className="relative aspect-square rounded-xl border border-blue-200 overflow-hidden group bg-white flex items-center justify-center shadow-sm"
                        >
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setRepFiles((p) => p.filter((_, i) => i !== idx))
                            }
                            className="absolute top-2 right-2 p-1.5 bg-rose-500 text-white rounded-lg shadow-md hover:bg-rose-600 transition-all z-10"
                            title="Hapus Foto"
                          >
                            <Trash size={12} />
                          </button>
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-6 pt-4 border-t border-slate-200">
                  <button
                    ...
                    disabled={isProcessing}
                    className="w-full sm:w-1/3 bg-emerald-50 text-emerald-600 border border-emerald-200 py-4 md:text-sm rounded-2xl font-black uppercase tracking-widest shadow-sm hover:bg-emerald-100 transition-all flex items-center justify-center gap-2"
                  >
                    {isProcessing ? (
                  ...
                  <button
                    type="submit"
                    disabled={isProcessing}
                    className="w-full sm:w-2/3 bg-blue-600 text-white py-4 md:text-lg rounded-2xl font-black uppercase tracking-widest shadow-2xl hover:bg-blue-700 transition-all flex items-center justify-center gap-3"
                  >
                    {isProcessing ? (
              ...
              >
                <div className="bg-emerald-50/50 p-6 md:p-8 rounded-2xl border border-emerald-100 space-y-6 w-full">
                  <SurveyInputRow label="Dokumentasi Foto / Video Lapangan">
                    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
                      {/* Preview Foto */}
                      {quickRepFiles &&
                        quickRepFiles.map((file, idx) => (
                          <div
                            key={idx}
                            className="relative aspect-square rounded-xl border border-emerald-200 overflow-hidden group bg-white flex items-center justify-center shadow-sm"
                          >
                            {file.type.startsWith('video/') ? (
                ...
                <div className="pt-4 border-t border-slate-200">
                  <button
                    type="submit"
                    disabled={isProcessing}
                    className="w-full bg-emerald-600 text-white py-4 md:text-lg rounded-2xl font-black uppercase tracking-widest shadow-2xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-3"
                  >
                    {isProcessing ? (
                      <Loader2 size={24} className="animate-spin" />
                    ) : (
            ...
            <form onSubmit={handleUnifiedSubmit} className="space-y-4">
              <SurveyInputRow label="Tanggal">
                <input
                  ...
                  value={uForm.tanggal}
                  onChange={(e) =>
                    setUForm((p) => ({ ...p, tanggal: e.target.value }))
                  }
                  className="w-full p-3 rounded-xl border bg-slate-50"
                />
              </SurveyInputRow>
              <SurveyInputRow label="Nama Jln/Gg./Blok">
                <input
                  type="text"
                  value={uForm.namaSegmen}
                  onChange={(e) =>
                    setUForm((p) => ({ ...p, namaSegmen: e.target.value }))
                  }
                  placeholder="Misal: Jl. Mawar / Segmen 1"
                  className="w-full p-3 rounded-xl border bg-slate-50"
                />
              </SurveyInputRow>
              <SurveyInputRow label="Titik Awal">
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder="Lat"
                    value={uForm.startLat}
                    onChange={(e) =>
                      setUForm((p) => ({ ...p, startLat: e.target.value }))
                    }
                    className="flex-1 p-3 rounded-xl border bg-slate-50"
                  />
                  <input
                    type="text"
                    placeholder="Lng"
                    value={uForm.startLng}
                    onChange={(e) =>
                      setUForm((p) => ({ ...p, startLng: e.target.value }))
                    }
                    className="flex-1 p-3 rounded-xl border bg-slate-50"
                  />
                  <button
                    type="button"
                    onClick={() => getUnifiedGPS('start')}
                    className="px-4 bg-blue-50 text-blue-600 font-bold rounded-xl text-[10px]"
                  >
                    GPS
                  </button>
                </div>
              ...
              <SurveyInputRow label="Titik Akhir">
                <div className="flex gap-2">
                  <input
                    ...
                    value={uForm.endLat}
                    onChange={(e) =>
                      setUForm((p) => ({ ...p, endLat: e.target.value }))
                    }
                    className="flex-1 p-3 rounded-xl border bg-slate-50"
                  />
                  <input
                    ...
                    value={uForm.endLng}
                    onChange={(e) =>
                      setUForm((p) => ({ ...p, endLng: e.target.value }))
                    }
                    className="flex-1 p-3 rounded-xl border bg-slate-50"
                  />
                  <button
                    type="button"
                    onClick={() => getUnifiedGPS('end')}
                    className="px-4 bg-rose-50 text-rose-600 font-bold rounded-xl text-[10px]"
                  >
                    GPS
                  </button>
                </div>
              ...
              <SurveyInputRow label="Panjang Eks.">
                <input
                  ...
                  value={uForm.panjang}
                  onChange={(e) =>
                    setUForm((p) => ({ ...p, panjang: e.target.value }))
                  }
                  className="w-full p-3 rounded-xl border bg-slate-50"
                />
              </SurveyInputRow>
              <SurveyInputRow label="Lebar Eks.">
                <input
                  ...
                  value={uForm.lebar}
                  onChange={(e) =>
                    setUForm((p) => ({ ...p, lebar: e.target.value }))
                  }
                  className="w-full p-3 rounded-xl border bg-slate-50"
                />
              </SurveyInputRow>
              <SurveyInputRow label="Jenis/Model Eks.">
                <input
                  type="text"
                  value={uForm.jenis_model_awal}
                  onChange={(e) =>
                    setUForm((p) => ({
                      ...p,
                      jenis_model_awal: e.target.value,
                    }))
                  }
                  className="w-full p-3 rounded-xl border bg-slate-50"
                />
              ...
              <SurveyInputRow label="Upload Data Ukur (CSV)">
                <input
                  type="file"
                  accept=".csv"
                  onChange={(e) => setUDataUkur(e.target.files[0])}
              ...
              <SurveyInputRow label="Catatan - Kendala - Kondisi">
                <textarea
                  rows="3"
                  value={uForm.noteDesc}
                  onChange={(e) =>
                    setUForm((p) => ({ ...p, noteDesc: e.target.value }))
                  }
                  className="w-full p-3 rounded-xl border bg-slate-50"
                ></textarea>
              </SurveyInputRow>
              <SurveyInputRow label="Dokumentasi Eks.">
                ...
                />
                {uMedia.length > 0 && (
                  <div className="text-[10px] mt-1.5 text-blue-600 font-bold">
                    {uMedia.length} file siap diunggah
                  </div>
                )}
              </SurveyInputRow>
              <button
                type="submit"
                disabled={isProcessing}
                className="w-full bg-blue-600 text-white py-4 rounded-2xl text-sm font-bold uppercase"
              >
          ...
          <div className="bg-white rounded-[32px] p-8 w-full max-w-lg shadow-2xl relative">
            ...
            <form onSubmit={handleSCurveTextSubmit} className="space-y-4">
              <div>
                ...
                <textarea
                  className="w-full p-4 rounded-2xl border font-mono bg-slate-50 outline-none focus:border-blue-400 text-sm"
                  rows="3"
                  value={sCurveForm.plan}
                  onChange={(e) =>
                    setSCurveForm({ ...sCurveForm, plan: e.target.value })
                  }
                  placeholder="Contoh: 0 - 5.5 - 12 - 25 - 45 - 70 - 100"
                ></textarea>
                {/* Live Preview Label M1, M2 */}
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {String(sCurveForm.plan || '')
              ...
              <button
                type="submit"
                disabled={isProcessing}
                className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold uppercase mt-4 shadow-md"
              >
                {isProcessing ? 'Proses...' : 'Simpan Kurva'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
          <div className="bg-white rounded-[32px] p-8 w-full max-w-md shadow-2xl relative">
            ...
            <form onSubmit={handleDocUpload} className="space-y-4">
              ...
              >
                ...
                {docFile ? (
                  <div className="flex flex-col items-center text-center">
                    <div className="p-3 bg-emerald-100 text-emerald-600 rounded-xl mb-3 shadow-sm">
                      <FileText size={28} />
                    </div>
                    <p className="text-sm font-black text-slate-800 line-clamp-1 px-4">
                      {docFile.name}
                    </p>
                    ...
                    <div
                      className={`p-3 rounded-xl mb-3 transition-colors ${
                        isDraggingDoc
                          ...
                          : 'bg-white shadow-sm border border-slate-200 text-slate-400 group-hover:text-blue-500'
                      }`}
                    >
                      <Upload size={28} />
                    </div>
              ...
              <div>
                <select
                  className="w-full p-4 rounded-xl border border-slate-200 bg-slate-50 font-bold outline-none focus:border-blue-400 transition-colors"
              ...
              <button
                type="submit"
                disabled={isProcessing}
                className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest mt-2 hover:bg-blue-700 transition-colors shadow-md"
              >
                {isProcessing ? 'Mengunggah...' : 'Simpan Dokumen'}
              </button>
            </form>
          </div>
        </div>
      ...
      {selectedLog && (
        <div
          className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[5000] p-4 md:p-8"
          onClick={() => setSelectedLog(null)}
        >
          <div
            className="bg-white rounded-[32px] w-full max-w-3xl shadow-2xl relative flex flex-col max-h-[90vh] overflow-hidden"
          ...
          >
            <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
              ...
              <button
                onClick={() => setSelectedLog(null)}
                className="p-2 bg-white rounded-full hover:bg-rose-50 hover:text-rose-500 transition-colors border border-slate-200 shadow-sm"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex gap-2 mb-6">
              ...
              <button
                onClick={() => {
                  if (
                    newCatName.trim() &&
                    !docCategories.includes(newCatName.trim())
                  ) {
                    ...
                    if (projectData)
                      supabaseClient
                        .from('projects')
                        .update({ doc_categories: newCats })
                        .eq('id', projectData.id)
                        .then();
                    setNewCatName('');
                  }
                }}
                className="px-4 bg-slate-800 text-white font-bold rounded-xl"
              >
                Add
              </button>
            </div>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {docCategories.map((cat, idx) => (
                <div
                  ...
                  className="flex justify-between p-3 border rounded-xl bg-white"
                >
      ...
      {moveDocConfig && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[2000] p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-xs shadow-2xl relative text-center">
            ...
            <select
              className="w-full p-3 rounded-xl border bg-slate-50 mb-6"
              value={moveDocConfig.newCategory || moveDocConfig.category}
              onChange={(e) =>
                setMoveDocConfig({
                  ...moveDocConfig,
      ...
      {deleteConfig && deleteConfig.type !== 'employee' && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[2000] p-4">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-sm shadow-2xl relative text-center">
            ...
            <p className="text-xs text-slate-500 mb-8 font-medium leading-relaxed">
              Data atau foto ini akan dihapus secara permanen dan tidak dapat
              dikembalikan lagi.
            </p>
            <div className="flex gap-3">
              <button
      ...
      {showSQLModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[2000] p-4">
          <div className="bg-white rounded-[32px] p-6 md:p-8 w-full max-w-2xl shadow-2xl relative">
            ...
            <button
              onClick={() => copyToClipboard(SQL_QUERY)}
