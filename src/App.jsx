          ...
          <div className="bg-white/80 backdrop-blur-md border border-slate-200 rounded-2xl shadow-xl p-5 pointer-events-auto w-max animate-in slide-in-from-left-4">
             <div className="flex justify-between items-start border-b border-slate-200 pb-3 mb-4">
               <div className="flex flex-col gap-1.5">
                 ...
                 <span className="text-[9px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded w-max uppercase tracking-widest border border-blue-100/50 shadow-sm">
                   {filterTahun === 'Semua' ? 'Semua Tahun' : `Tahun ${filterTahun}`}
                 </span>
               </div>
               <button onClick={() => setShowLegend(false)} className="text-slate-500 hover:text-rose-500 transition-colors p-1.5 bg-slate-100/50 rounded-lg hover:bg-slate-200 ml-8 shrink-0" title="Sembunyikan Legenda"><X size={14} /></button>
             </div>
        ...
        <div className="absolute top-24 right-4 md:top-20 md:right-6 bottom-4 z-[9999] w-[300px] md:w-[340px] bg-white/80 backdrop-blur-md border border-slate-200 rounded-2xl shadow-xl flex flex-col pointer-events-auto overflow-hidden animate-in slide-in-from-right-4">
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                  ...
                  <div key={layer.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden transition-all">
                     ...
                     >
                        <div className="flex items-center gap-2">
                           ...
                           <span className="font-bold text-slate-700 text-xs truncate max-w-[140px]" title={layer.name}>{layer.name || 'Area / Jalur'}</span>
                        </div>
                        <div className="flex items-center gap-2">
             ...
             <div className="flex gap-2 mt-4">
               <button onClick={() => {
                   const newId = `global-${Date.now()}`;
                   setGlobalLayers(prev => [...prev, { id: newId, name: `Jalur Air ${prev.length + 1}`, type: 'line', color: '#0ea5e9', isDashed: false, points: [] }]);
               ...
               <button onClick={() => {
                   ...
                   setGlobalLayers(prev => [...prev, { id: newId, name: `Kolam Retensi ${prev.length + 1}`, type: 'polygon', color: '#14b8a6', isDashed: false, points: [] }]);
                   setExpandedGlobalLayerId(newId);
